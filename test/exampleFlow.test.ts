...
describe('basic example flow', () => {
  ...
  it('generates Markdown for all editable VS Code Agent Flow file types', async () => {
    ...
    expect(routerAgent?.content).toContain('<!--agent-flow:begin artifact-ref action="write" path=".agent-output/triage.md"-->');
    ...
    expect(prompt?.content).toContain('<!--agent-flow:begin artifact-ref action="read" path=".agent-output/triage.md"-->');
