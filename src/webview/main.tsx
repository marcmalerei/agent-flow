...
import { autoFitViewportNodeIds, meaningfulFlowNodeIds } from './meaningfulFlow';
...
declare global {
  interface Window {
    __AGENTFLOW_STATE__: State;
    __AGENTFLOW_APP_BOOTED__?: boolean;
    __AGENTFLOW_VSCODE_API__?: { postMessage(message: unknown): void };
...
const nodeTypeIcons: Record<PipelineNodeType, string> = {
  agent: 'hubot',
  prompt: 'comment-discussion',
  instruction: 'list-tree',
  skill: 'tools',
  role: 'person',
  artifact: 'file',
  gate: 'pass',
  hook: 'debug-disconnect',
  handoff: 'arrow-swap',
  'mcp-server': 'server-process',
};
...
function deriveState(pipeline: AgentPipeline, previous: State): State {
  ...
  return {
    ...
    generatedFiles: generateFiles(pipeline).map((file) => ({
      path: file.path,
      kind: file.kind,
    })),
...
function App() {
  ...
  useEffect(() => {
    const timers = [0, 100, 500, 1_500].map((delay) =>
      window.setTimeout(() => {
        vscode?.postMessage({
          command: 'webviewReady',
          bootId: webviewBootId,
  ...
  const commitDraft = useCallback((updater: (pipeline: AgentPipeline) => AgentPipeline, nextSelectedId?: string, dirtyNodeIds: string[] = []) => {
    setDraft((pipeline) => {
      ...
      setState((previous) => {
        ...
        return dirtyNodeIds.length
          ? {
              ...derived,
              nodeRuntime: markNodeRuntimeDirty(derived.nodeRuntime, dirtyNodeIds),
            }
          : derived;
      });
  ...
  const pasteSelection = useCallback(() => {
    ...
    setDraft((pipeline) => {
      ...
      setState((previous) => {
        ...
        return {
          ...derived,
          nodeRuntime: markNodeRuntimeDirty(derived.nodeRuntime, result.selectedIds),
        };
      });
  ...
  const activityPlayback = useMemo(
    () =>
      deriveActivityPlaybackState(state.activityEvents ?? [], {
        now: activityClock,
        paused: activityPaused,
        replayEventId,
      }),
    [activityClock, activityPaused, replayEventId, state.activityEvents],
  );
  ...
  const nodes: RenderedNode[] = useMemo(
    () =>
      normalizeGraphNodePositions(
        draft.nodes.map((node) => {
          ...
          return {
            ...
            height: size.height,
            data: {
              label: graphNodeDisplayLabel(node),
              fullLabel: graphNodeFullLabel(node),
              type: node.type,
              tokenBadge: formatTokenBadge(estimateNodeTokenCount(draft, node)),
              tokenColor: nodeTypeColor(node.type),
              activity: activityByNode.get(node.id),
              runtimeStatus: state.nodeRuntime?.[node.id]?.status,
              dirty: state.nodeRuntime?.[node.id]?.dirty,
              attention: risky.has(node.id),
              ...handlePositions,
            },
            style: {
              '--agentflow-node-color': nodeColor,
  ...
  const loopEdgeIds = useMemo(() => {
    ...
    const cycles = findCycles(
      draft.nodes,
      visibleEdges.map((edge) => ({ from: edge.source, to: edge.target })),
    );
  ...
  const edges: RenderedEdge[] = useMemo(
    () =>
      visibleEdges.map((edge) => {
        ...
        const classNames = [
          ...
          edgeReadingLevelClass(edge, graphReadingLevel, {
            active: edgeActive,
  ...
  const updateNode = (nodeId: string, patch: Partial<PipelineNode>) => {
    if (Object.prototype.hasOwnProperty.call(patch, 'label')) {
      commitDraft(
        (pipeline) => renamePipelineNodeLabel(pipeline, nodeId, String(patch.label ?? '')),
        undefined,
    ...
    commitDraft(
      (pipeline) => ({
        ...pipeline,
        nodes: pipeline.nodes.map((node) => (node.id === nodeId ? applyNodePatch(node, patch) : node)),
      }),
      undefined,
      [nodeId],
    );
  ...
  const applyValidationQuickFix = useCallback(
  ...
  const openConflictDiff = useCallback(() => {
    ...
    vscode?.postMessage({
      ...
      pipeline: draftRef.current,
      nodeId: editingConflict.nodeId,
    });
  }, [editingConflict]);
  ...
  return (
    <FlowApp
      ...
      activityPlayback={activityPlayback}
      ...
      canPaste={copiedIds.length > 0 || Boolean(selectedId)}
      undoLast={undoLast}
      redoLast={redoLast}
      copySelection={copySelection}
      pasteSelection={pasteSelection}
      setActivityPaused={setActivityPaused}
      setReplayEventId={setReplayEventId}
      setActiveTab={setActiveTab}
      setBottomOpen={setBottomOpen}
      setGraphMode={setGraphMode}
      setGraphReadingLevel={setGraphReadingLevel}
      ...
      addNode={addNode}
      onApplyExternalChanges={applyExternalChanges}
      onKeepLocalEdit={keepLocalEdit}
      onOpenConflictDiff={openConflictDiff}
      onCancelLocalEdit={cancelLocalEdit}
      onApplyValidationQuickFix={applyValidationQuickFix}
    />
  );
}
...
function FlowApp({
  ...
  deleteNodes,
  addNode,
  onApplyExternalChanges,
  onKeepLocalEdit,
  onOpenConflictDiff,
  onCancelLocalEdit,
  onApplyValidationQuickFix,
}: {
  state: State;
  draft: AgentPipeline;
  ...
  activityHud: ActivityHudState;
  activityTrail: ActivityTrailItem[];
  activityPlayback: ReturnType<typeof deriveActivityPlaybackState>;
  ...
  syncBanner?: SyncTrustBannerState;
  canUndo: boolean;
  canRedo: boolean;
  canPaste: boolean;
  undoLast: () => void;
  ...
  setReplayEventId: React.Dispatch<React.SetStateAction<string | undefined>>;
  setActiveTab: (tab: BottomTab) => void;
  setBottomOpen: (open: boolean) => void;
  setGraphMode: (mode: GraphMode) => void;
  setGraphReadingLevel: (level: GraphReadingLevel) => void;
  setInspectorOpen: (open: boolean) => void;
  setSelectedId: (id: string) => void;
  setSyncBanner: React.Dispatch<React.SetStateAction<SyncTrustBannerState | undefined>>;
  updateNode: (nodeId: string, patch: Partial<PipelineNode>) => void;
  connectNodes: (sourceId: string, targetId: string) => void;
  applyConnection: (sourceId: string, targetId: string, kind: ConnectionIntentKind) => void;
  deleteNodes: (nodeIds: string[]) => void;
  deleteEdges: (edgeIds: string[]) => void;
  addNode: (node: PipelineNode, connectFrom?: string, intent?: ConnectionIntentKind) => void;
  onApplyExternalChanges: () => void;
  onKeepLocalEdit: () => void;
  onOpenConflictDiff: () => void;
  onCancelLocalEdit: () => void;
  onApplyValidationQuickFix: (action: ValidationAction | undefined) => void;
}) {
  ...
  const diagnosticsResize = useResizablePanel({
    axis: 'y',
    initialSize: 250,
    invert: true,
    min: 180,
  ...
  const initialOverviewLocked = useRef(true);
  ...
  const setGraphViewport = useCallback((next: GraphViewport, userInteracted = false) => {
    if (userInteracted) {
      userViewportInteracted.current = true;
  ...
  const fitViewport = useCallback(
    ...
    [draft, flowNodeSignature, graphBounds, graphFocusMode, setGraphViewport, visibleNodes],
  );

  useEffect(() => {
    const report = (reason: string) => {
      const status = postFlowRenderStatus(
        canvasRef.current,
        state.stateVersion,
        visibleNodes.map((node) => node.id),
        visibleEdgesForFilters.length,
        reason,
        viewportRef.current,
        graphBounds,
      );
    ...
    if (
      shouldAutoFitGraph({
        previousSignature: lastFitSignature.current,
        nextSignature: flowNodeSignature,
        userInteracted: userViewportInteracted.current,
        reason: 'structure',
      })
    ) {
      fitViewport();
  ...
  const nodeCreationPosition = useCallback(() => {
    const rect = canvasRef.current?.getBoundingClientRect();
    return rect ? screenToGraphPosition({ x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 }, rect, viewport) : { x: 120, y: 120 };
  }, [viewport]);
  const beginNodeCreation = (type: PipelineNodeType, connectFrom?: string) => {
    setPendingNodeConnection(undefined);
    setCreationFeedback(undefined);
    setNodeCreationDraft({
      connectFrom,
      description: '',
      name: `new ${type}`,
      type,
  ...
  const confirmNodeCreation = () => {
    ...
    const targetNode = createPipelineNode(draft, nodeCreationDraft.type, position, {
      description: nodeCreationDraft.description,
      name: nodeCreationDraft.name,
    });
  ...
  const stepGraphSearch = useCallback(
    (direction: 1 | -1) => {
  ...
  const fitFullGraph = useCallback(() => {
    initialOverviewLocked.current = false;
    fitViewport('full', true);
  }, [fitViewport]);
  const changeGraphFocusMode = useCallback((mode: GraphFocusMode) => {
    initialOverviewLocked.current = false;
  ...
  return (
    <div className={`app ${graphModeClassNames[graphMode]} ${readingLevelClassNames[graphReadingLevel]} ${bottomOpen ? 'bottom-open' : 'bottom-collapsed'} ${inspectorOpen ? 'inspector-open' : 'inspector-closed'}`} style={appStyle}>
      <header className="toolbar">
        ...
        <div className="toolbar-workflow">
          ...
          <ActivityPlaybackControls
            paused={activityPaused}
            mode={activityPlayback.mode}
            replayEventId={activityPlayback.replayEventId}
            latestEventId={activityTrail[0]?.id}
            onClear={() => {
              setReplayEventId(undefined);
              vscode?.postMessage({ command: 'clearActivity' });
            }}
            onPauseToggle={() => setActivityPaused((paused) => !paused)}
            onReplayLatest={() => activityTrail[0] && setReplayEventId(activityTrail[0].id)}
        ...
        <div className="toolbar-actions">
          <div className="toolbar-primary-actions">
            <VSCodeButton
              className={`compact follow-live-toggle${followLiveActivity ? ' active' : ''}`}
              icon="target"
              aria-pressed={followLiveActivity}
              onClick={() =>
                setFollowLiveActivity((enabled) => {
                  if (!enabled) userViewportInteracted.current = false;
                  return !enabled;
                })
              }
              title="Follow live activity without changing zoom"
            >
              Follow live
            </VSCodeButton>
            <VSCodeButton className="compact" icon="question" aria-keyshortcuts="?" onClick={() => setShortcutsOpen((open) => !open)} title="Keyboard shortcuts">
              Shortcuts
            </VSCodeButton>
            <div className="add-node-menu" ref={addNodeMenuRef}>
              <VSCodeButton
                className="compact"
                icon="add"
                aria-haspopup="menu"
              ...
              {addNodeMenuOpen && (
                <div className="add-node-popover" role="menu" aria-label="Add node">
                  {nodeCreationDraft && nodeCreationPreview ? (
                    <NodeCreationForm draft={nodeCreationDraft} preview={nodeCreationPreview} connectFromLabel={nodeCreationDraft.connectFrom ? draft.nodes.find((node) => node.id === nodeCreationDraft.connectFrom)?.label : undefined} onCancel={() => setNodeCreationDraft(undefined)} onChange={setNodeCreationDraft} onCreate={confirmNodeCreation} />
                  ) : (
                    <>
                      {nodePaletteGroups.map((group) => (
                        <section className="node-palette-group" key={group.label}>
                          <h3>{group.label}</h3>
                          {group.types.map((type) => (
                            <div className="node-palette-item" key={type}>
                              <button type="button" role="menuitem" onClick={() => beginNodeCreation(type)}>
                                <Codicon name={nodeTypeIcons[type]} />
                                <span>{nodeTypeLabel(type)}</span>
                                <small>{nodeTypeDescription(type)}</small>
                              </button>
                              {selected && (
                                <button type="button" className="node-palette-connect" onClick={() => beginNodeCreation(type, selected.id)} title={`Connect from selected ${selected.label}`}>
                                  <Codicon name="link" />
                                  <span>Connect from selected</span>
                                </button>
                              )}
                            </div>
                          ))}
                      ...
                      {pendingNodeConnection && (
                        <ConnectionIntentChooser
                          pending={pendingNodeConnection}
                          source={draft.nodes.find((node) => node.id === pendingNodeConnection.sourceId)}
                          onCancel={() => setPendingNodeConnection(undefined)}
                          onCreateOnly={() => {
                            addNode(pendingNodeConnection.targetNode);
                            setCreationFeedback(`Created ${nodeFileSummary(pendingNodeConnection.targetNode)}`);
                            setPendingNodeConnection(undefined);
                            setAddNodeMenuOpen(false);
                          }}
                          onCreateAndConnect={(kind) => {
                            addNode(pendingNodeConnection.targetNode, pendingNodeConnection.sourceId, kind);
                            setCreationFeedback(`Created ${nodeFileSummary(pendingNodeConnection.targetNode)}`);
                            setPendingNodeConnection(undefined);
                            setAddNodeMenuOpen(false);
                          }}
          ...
          <div className="toolbar-secondary-actions">
            <VSCodeButton className="compact" icon="discard" aria-keyshortcuts="Control+Z Meta+Z" onClick={undoLast} disabled={!canUndo} title="Undo last graph change">
              Undo
            </VSCodeButton>
            <VSCodeButton className="compact" icon="redo" aria-keyshortcuts="Control+Y Meta+Y" onClick={redoLast} disabled={!canRedo} title="Redo last graph change">
              Redo
            </VSCodeButton>
            <VSCodeButton className="compact" icon="copy" aria-keyshortcuts="Control+C Meta+C" onClick={copySelection} disabled={!selectedId} title="Copy selected node">
              Copy
            </VSCodeButton>
            <VSCodeButton className="compact" icon="files" aria-keyshortcuts="Control+V Meta+V" onClick={pasteSelection} disabled={!canPaste} title="Paste copied node">
              Paste
            </VSCodeButton>
          </div>
        </div>
        <div className="toolbar-status-row">
          ...
          {creationFeedback && (
            <span className="creation-feedback" role="status">
              <Codicon name="pass" />
              {creationFeedback}
            </span>
          )}
      ...
      {syncBanner && (
        <SyncTrustBanner
          banner={syncBanner}
          onDismiss={() => setSyncBanner(undefined)}
          onOpenDiagnostics={() => {
            setBottomOpen(true);
            setActiveTab('validation');
          }}
          onReloadGraph={() =>
            vscode?.postMessage({
              command: 'runCommand',
              name: 'agentflow.scanWorkspace',
            })
          }
          onReviewChanges={() => (editingConflict ? onOpenConflictDiff() : (setBottomOpen(true), setActiveTab('files')))}
        />
      )}
      {showFirstRunGuide && (
        <FirstRunGuideCallout
          onDismiss={dismissFirstRunGuide}
          onPlayDemo={() =>
            vscode?.postMessage({
              command: 'runCommand',
              name: 'agentflow.startGuidedDemo',
            })
          }
          onSelectImplementer={() => {
            setSelectedId('implementer');
            setInspectorOpen(true);
          }}
        />
      )}
      <NativeGraph
        ...
        onGraphFocusModeChange={changeGraphFocusMode}
        canvasRef={canvasRef}
        nodes={visibleNodes}
        edges={visibleEdgesForFilters}
        selectedId={selectedId}
        ...
        searchIndex={graphSearchIndex}
        typeFilterOptions={graphTypeOptions}
        selectedGraphTypes={selectedGraphTypes}
        graphFilterEmpty={graphFilterEmpty}
        artifactSummary={artifactSummary}
        onActivitySelect={(item) => {
          setReplayEventId(item.id);
          openActivityForNode(item.nodeId ?? item.targetNodeId);
        }}
        onViewportChange={setGraphViewport}
        onFit={fitFullGraph}
        onFitMeaningfulFlow={fitMeaningfulFlow}
        onFitSelectedNeighborhood={fitSelectedNeighborhood}
        onJumpActive={jumpToActive}
        onJumpProblem={jumpToProblem}
        onJumpSelected={jumpToSelected}
        ...
        onClearFocus={() => setSelectedId('')}
        onTypeFilterChange={setSelectedGraphTypes}
        onOpenSelected={() => selectedId && setInspectorOpen(true)}
        onCanvasClick={() => setInspectorOpen(false)}
        onDeleteSelected={() => selectedId && deleteNodes([selectedId])}
        onSearchChange={updateGraphSearch}
        onSearchClear={clearGraphSearch}
        onSearchStep={stepGraphSearch}
      />
      ...
      {inspectorOpen && (
        <aside className="inspector">
          <Inspector node={selected} pipeline={draft} toolOptions={state.toolOptions} runtime={selected ? state.nodeRuntime?.[selected.id] : undefined} findings={state.findings.filter((finding) => finding.nodeId === selectedId)} conflict={editingConflict} onChange={updateNode} onConnect={applyConnection} onApplyExternalChanges={onApplyExternalChanges} onKeepLocalEdit={onKeepLocalEdit} onOpenConflictDiff={onOpenConflictDiff} onCancelLocalEdit={onCancelLocalEdit} />
        </aside>
      )}
      <section className="bottom">
        {bottomOpen && <div className="panel-resize-handle diagnostics-resize-handle" role="separator" aria-label="Resize diagnostics panel" aria-orientation="horizontal" aria-valuemin={diagnosticsResize.min} aria-valuemax={diagnosticsResize.max} aria-valuenow={diagnosticsResize.size} tabIndex={0} {...diagnosticsResize.resizeHandleProps} />}
        <VSCodeButton className="collapse" icon={bottomOpen ? 'chevron-down' : 'chevron-right'} onClick={() => setBottomOpen(!bottomOpen)}>
          {bottomOpen ? 'Hide diagnostics' : 'Show diagnostics'}
        </VSCodeButton>
        {bottomOpen && (
          <Bottom
            state={state}
            activeTab={activeTab}
            setActiveTab={setActiveTab}
            onApplyQuickFix={onApplyValidationQuickFix}
            onSelectNode={(nodeId) => {
              setSelectedId(nodeId);
              setInspectorOpen(true);
            }}
...
function syncStatusState(kind: SyncStatusKind, detail?: string): SyncStatusState {
  if (kind === 'saving')
    return {
      ...
      label: 'Saving',
      icon: 'sync',
      detail: detail ?? 'Writing webview edits to workspace files.',
    };
  if (kind === 'reading-workspace')
    return {
      kind,
      label: 'Reading workspace',
      icon: 'search',
      detail: detail ?? 'Refreshing graph data from workspace files.',
    };
  if (kind === 'external-change')
    return {
      ...
      icon: 'repo-pull',
      detail: detail ?? 'Workspace files changed outside this webview.',
    };
  if (kind === 'parse-issue')
    return {
      kind,
      label: 'Parse issue',
      icon: 'warning',
      detail: detail ?? 'A workspace refresh reported parse or graph issues.',
    };
  if (kind === 'stale-view')
    return {
      kind,
      label: 'Stale view',
      icon: 'history',
      detail: detail ?? 'Keeping the last known graph because a refresh looked incomplete.',
    };
  return {
    kind,
    label: 'Synced',
    icon: 'pass',
    detail: detail ?? 'Graph and workspace files are in sync.',
  };
}
...
function syncBannerForRemoteMerge(reason: RemoteStateMergeResult<State>['reason'], sourceReason: string): SyncTrustBannerState | undefined {
  ...
  if (reason === 'external-conflict' || reason === 'local-dirty')
    return {
      kind: 'external-change',
      icon: 'repo-pull',
      title: 'External changes detected',
      detail: reason === 'external-conflict' ? 'The selected node changed outside Agent Flow while you had local edits.' : `Workspace changes arrived from ${sourceReason}; your local graph edit is still preserved.`,
    };
  return undefined;
}
...
function SyncStatusIndicator({ status }: { status: SyncStatusState }) {
  return (
    <span className={`autosave-status sync-status-${status.kind}`} title={status.detail}>
      <Codicon name={status.icon} />
      {status.label}
    </span>
  );
}
...
function SyncTrustBanner({ banner, onDismiss, onOpenDiagnostics, onReloadGraph, onReviewChanges }: { banner: SyncTrustBannerState; onDismiss: () => void; onOpenDiagnostics: () => void; onReloadGraph: () => void; onReviewChanges: () => void }) {
  return (
    <section className={`sync-trust-banner sync-trust-${banner.kind}`} role="status" aria-live="polite">
      ...
      <div className="sync-trust-actions">
        <VSCodeButton className="compact" icon="diff" onClick={onReviewChanges}>
          Review changes
        </VSCodeButton>
        <VSCodeButton className="compact" icon="refresh" onClick={onReloadGraph}>
          Reload graph
        </VSCodeButton>
        <VSCodeButton className="compact" icon="list-selection" onClick={onOpenDiagnostics}>
          Open diagnostics
        </VSCodeButton>
...
function FirstRunGuideCallout({ onDismiss, onPlayDemo, onSelectImplementer }: { onDismiss: () => void; onPlayDemo: () => void; onSelectImplementer: () => void }) {
  return (
    <aside className="first-run-guide" aria-label="First-run guide">
      <header>
        <Codicon name="sparkle" />
        <strong>First-run guide</strong>
        <VSCodeIconButton type="button" icon="close" title="Dismiss first-run guide" aria-label="Dismiss first-run guide" onClick={onDismiss} />
      </header>
      <div className="first-run-guide-steps">
        <button type="button" onClick={onSelectImplementer}>
          <Codicon name="target" />
          <span>Select implementer</span>
        </button>
        <button type="button" onClick={onSelectImplementer}>
          <Codicon name="references" />
          <span>Create artifact reference</span>
        </button>
...
function KeyboardShortcutsPopover({ onClose }: { onClose: () => void }) {
  ...
  return (
    <aside className="shortcut-help keyboard-shortcuts-popover" role="dialog" aria-label="Keyboard shortcuts" id="agentflow-keyboard-shortcuts">
      <header>
...
function ActivityHud({ onOpen, state }: { onOpen: () => void; state: ActivityHudState }) {
  ...
  return (
    <button type="button" className={`activity-hud activity-hud-${state.mode}`} onClick={onOpen} title={`${label}. ${detail}`}>
      <Codicon name={icon} />
      <span>{label}</span>
      <small>{state.recentCount ? `${state.recentCount} recent · ${capability}` : capability}</small>
      {state.now && (
        <span className="activity-now-card" data-event-id={state.now.eventId}>
...
function ActivityPlaybackControls({ latestEventId, mode, onClear, onPauseToggle, onReplayLatest, onStopReplay, paused, replayEventId }: { latestEventId?: string; mode: string; onClear: () => void; onPauseToggle: () => void; onReplayLatest: () => void; onStopReplay: () => void; paused: boolean; replayEventId?: string }) {
  return (
    <div className={`activity-playback-controls activity-playback-${mode}`} role="group" aria-label="Activity playback controls">
...
function GraphModeSwitch({ mode, onChange }: { mode: GraphMode; onChange: (mode: GraphMode) => void }) {
  return (
    <div className="graph-mode-switch" role="group" aria-label="Graph workflow mode">
      {graphModes.map((option) => (
        <button type="button" key={option.id} className={mode === option.id ? 'active' : ''} aria-pressed={mode === option.id} title={option.description} onClick={() => onChange(option.id)}>
...
function NodeCreationForm({ connectFromLabel, draft, onCancel, onChange, onCreate, preview }: { connectFromLabel?: string; draft: NodeCreationDraft; onCancel: () => void; onChange: (draft: NodeCreationDraft) => void; onCreate: () => void; preview: NodeCreationPreview }) {
  ...
  return (
    <section className="node-creation-form" aria-label="Create node">
      <header>
        <Codicon name={nodeTypeIcons[draft.type]} />
        <strong>Create {nodeTypeLabel(draft.type)}</strong>
      </header>
      <label>
        Name or id
        <input value={draft.name} onChange={(event: any) => onChange({ ...draft, name: event.target.value })} placeholder={`new ${draft.type}`} />
      </label>
      <label>
        Description
        <textarea value={draft.description} onChange={(event: any) => onChange({ ...draft, description: event.target.value })} rows={3} placeholder="Optional short purpose." />
      </label>
      ...
      <div className="node-creation-actions">
        <VSCodeButton className="compact" icon="close" onClick={onCancel}>
          Cancel
        </VSCodeButton>
        <VSCodeButton className="compact" icon="add" onClick={onCreate}>
          Create
        </VSCodeButton>
      </div>
    </section>
...
function GraphSearchControl({ matches, onChange, onClear, onStep, query, searchIndex }: { matches: GraphSearchResult[]; onChange: (query: string) => void; onClear: () => void; onStep: (direction: 1 | -1) => void; query: string; searchIndex: number }) {
  ...
  return (
    <div className="graph-search-control" role="search">
      ...
      {query && (
        <div className="graph-search-results" aria-live="polite">
          ...
          <button type="button" title="Previous graph search result" aria-label="Previous graph search result" disabled={!matches.length} onClick={() => onStep(-1)}>
            <Codicon name="chevron-up" />
          </button>
          <button type="button" title="Next graph search result" aria-label="Next graph search result" disabled={!matches.length} onClick={() => onStep(1)}>
            <Codicon name="chevron-down" />
          </button>
          <button type="button" title="Clear graph search" aria-label="Clear graph search" onClick={onClear}>
            <Codicon name="close" />
          </button>
        </div>
      )}
...
function GraphTypeFilters({ onChange, options, selectedTypes }: { onChange: (types: PipelineNodeType[]) => void; options: GraphTypeFilterOption[]; selectedTypes: PipelineNodeType[] }) {
  ...
  return (
    <div className="graph-type-filters" aria-label="Graph type filters">
      {options.map((option) => (
        <button type="button" key={option.type} className={selected.has(option.type) ? 'active' : ''} aria-pressed={selected.has(option.type)} onClick={() => toggle(option.type)} title={`Show or hide ${option.label.toLowerCase()}`}>
          <span>{option.label}</span>
...
function GraphReadingLevelSwitch({ level, onChange }: { level: GraphReadingLevel; onChange: (level: GraphReadingLevel) => void }) {
  return (
    <div className="graph-reading-level-switch" role="group" aria-label="Graph reading level">
      {graphReadingLevels.map((option) => (
        <button type="button" key={option.id} className={level === option.id ? 'active' : ''} aria-pressed={level === option.id} title={option.description} onClick={() => onChange(option.id)}>
          <Codicon name={option.icon} />
          <span>{option.label}</span>
        </button>
      ))}
...
function GraphFocusModeSwitch({ mode, onChange }: { mode: GraphFocusMode; onChange: (mode: GraphFocusMode) => void }) {
  return (
    <div className="graph-focus-mode-switch" role="group" aria-label="Graph focus mode">
      {graphFocusModes.map((option) => (
        <button type="button" key={option.id} className={mode === option.id ? 'active' : ''} aria-pressed={mode === option.id} title={option.description} onClick={() => onChange(option.id)}>
          <Codicon name={option.icon} />
          <span>{option.label}</span>
        </button>
...
function ArtifactRelationshipSummary({ onSelectNode, summary }: { onSelectNode: (nodeId: string) => void; summary: ArtifactRelationshipSummaryModel }) {
  return (
    <aside className="artifact-relationship-summary" aria-label="Artifact relationships">
      <header>
        <Codicon name="file" />
        ...
        <div>
          {nodes.map((node) => (
            <button type="button" key={node.id} onClick={() => onSelectNode(node.id)}>
              <Codicon name={nodeTypeIcons[node.type]} />
...
function NativeGraph({ canvasRef, graphMode, graphReadingLevel, graphFocusMode, nodes, edges, selectedId, selectedNode, activeNodeIds, problemNodeIds, activityTrail, replayEventId, viewport, graphBounds, emptyState, recoveryState, searchQuery, searchMatches, searchIndex, typeFilterOptions, selectedGraphTypes, graphFilterEmpty, artifactSummary, onActivitySelect, onViewportChange, onFit, onFitMeaningfulFlow, onFitSelectedNeighborhood, onGraphFocusModeChange, onGraphReadingLevelChange, onJumpActive, onJumpProblem, onJumpSelected, onJumpStart, onOpenDiagnostics, onNodeClick, onSelectNode, onClearFocus, onTypeFilterChange, onOpenSelected, onCanvasClick, onDeleteSelected, onSearchChange, onSearchClear, onSearchStep }: NativeGraphProps) {
  ...
  return (
    <main className={`canvas native-graph ${graphModeClassNames[graphMode]} ${readingLevelClassNames[graphReadingLevel]}`} ref={canvasRef} tabIndex={0} aria-label="Agent Flow graph canvas" aria-describedby="agentflow-keyboard-shortcuts" aria-keyshortcuts="ArrowLeft ArrowRight ArrowUp ArrowDown Enter Escape F Backspace Delete" onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={endPan} onPointerCancel={endPan} onWheel={onWheel} onKeyDown={onKeyDown}>
      ...
      {selectedId && selectedNode && (
        <div className="graph-focus-chip" aria-live="polite">
          <Codicon name="target" />
          <span>Focused: {selectedNode.label}</span>
          <button type="button" title="Clear graph focus" aria-label="Clear graph focus" onClick={onClearFocus}>
            <Codicon name="close" />
          </button>
        </div>
      )}
      ...
      {graphFilterEmpty && (
        <div className="graph-filter-empty" role="status">
          <Codicon name="filter" />
          <span>No graph nodes match the active graph filters.</span>
          <button
            type="button"
            onClick={() => {
              onTypeFilterChange(typeFilterOptions.map((option) => option.type));
              onGraphFocusModeChange('full');
            }}
          >
            Show full graph
          </button>
      ...
      <div
        ...
        style={{
          transform: graphTransform(viewport),
      ...
      >
        <svg className="graph-edge-layer" width={graphBounds.width} height={graphBounds.height} viewBox={`0 0 ${graphBounds.width} ${graphBounds.height}`} aria-hidden="true">
            ...
            {edges.map((edge) => {
              ...
              return (
                <React.Fragment key={edge.id}>
                  ...
                  <marker id={edgeMarkerId(edge.id)} markerWidth="3.5" markerHeight="3.5" refX="3.2" refY="1.75" orient="auto" markerUnits="userSpaceOnUse">
                    <path
                      d="M 0 0 L 3.5 1.75 L 0 3.5 z"
                      fill={edgeMarkerColor(
                        target
                          ? ({
                              id: target.id,
          ...
          {edges.map((edge) => (
            <GraphEdge key={edge.id} edge={edge} nodesById={nodesById} selectedId={selectedId} />
          ))}
        </svg>
      ...
      {activityTrail.length > 0 && (
        <div className="activity-trail" aria-label="Recent activity trail">
          {activityTrail.map((item) => (
            <button type="button" key={item.id} className={item.id === replayEventId ? 'active' : undefined} aria-label={`Replay activity ${item.label}: ${item.summary}`} aria-pressed={item.id === replayEventId} title={`${item.label}: ${item.summary}`} onClick={() => onActivitySelect(item)}>
      ...
      <div className="native-controls" aria-label="Graph controls">
        <button
          ...
          onClick={() =>
            onViewportChange(
              {
                ...viewport,
                zoom: clamp(viewport.zoom * 1.18, nativeGraphMinZoom, nativeGraphMaxZoom),
              },
              true,
            )
          }
        ...
        <button type="button" title="Fit meaningful flow" aria-label="Fit meaningful flow" onClick={onFitMeaningfulFlow}>
          <Codicon name="symbol-interface" />
        </button>
        <button type="button" title="Fit graph" aria-label="Fit graph" aria-keyshortcuts="F" onClick={onFit}>
          <Codicon name="screen-full" />
        </button>
      </div>
      <div className="graph-navigation-landmarks" aria-label="Graph navigation landmarks">
        <button type="button" title="Jump to start" aria-label="Jump to start" disabled={!nodes.length} onClick={onJumpStart}>
          <Codicon name="debug-start" />
        </button>
        <button type="button" title="Jump to active node" aria-label="Jump to active node" disabled={!activeNodeIds.length} onClick={onJumpActive}>
          <Codicon name="pulse" />
        </button>
        <button type="button" title="Jump to selected node" aria-label="Jump to selected node" disabled={!selectedId} onClick={onJumpSelected}>
          <Codicon name="target" />
        </button>
        <button type="button" title="Fit meaningful flow" aria-label="Fit meaningful flow" disabled={!nodes.length} onClick={onFitMeaningfulFlow}>
          <Codicon name="symbol-interface" />
        </button>
        <button type="button" title="Fit selected neighborhood" aria-label="Fit selected neighborhood" disabled={!selectedId} onClick={onFitSelectedNeighborhood}>
          <Codicon name="symbol-interface" />
        </button>
        <button type="button" title="Jump to first problem" aria-label="Jump to first problem" disabled={!problemNodeIds.length} onClick={onJumpProblem}>
          <Codicon name="warning" />
        </button>
        <button type="button" title="Fit graph" aria-label="Fit all graph nodes" onClick={onFit}>
          <Codicon name="screen-full" />
        </button>
      </div>
...
function GraphOverview({ activeNodeIds, canvasRef, graphBounds, nodes, onFit, problemNodeIds, selectedId, viewport }: { activeNodeIds: string[]; canvasRef: React.RefObject<HTMLElement>; graphBounds: GraphBounds; nodes: RenderedNode[]; onFit: () => void; problemNodeIds: string[]; selectedId: string; viewport: GraphViewport }) {
  ...
  const overview = graphOverviewMetrics(graphBounds, viewport, canvasSize, {
    width: 168,
    height: 96,
  });
  ...
  return (
    <button type="button" className="graph-overview" aria-label="Graph overview" title="Fit graph overview" onClick={onFit}>
...
function FlowRecoveryStateView({ onOpenDiagnostics, onRetry, state }: { onOpenDiagnostics: () => void; onRetry: () => void; state: GraphRecoveryState }) {
  ...
  return (
    <section className={`flow-recovery-state flow-recovery-${state.kind}`} aria-live="polite">
      <div className="flow-recovery-card">
        ...
        {state.actionLabels.length > 0 && (
          <div className="flow-recovery-actions">
            {state.actionLabels.map((label) => (
              <VSCodeButton key={label} className="compact" icon={label === 'Copy debug snapshot' ? 'copy' : label === 'Open diagnostics' ? 'list-selection' : 'refresh'} onClick={() => runAction(label)}>
                {label}
              </VSCodeButton>
            ))}
          </div>
...
function FlowEmptyStateView({ state }: { state: FlowEmptyState }) {
  ...
  return (
    <section className="flow-empty-state" aria-live="polite">
      <div className="flow-empty-card">
        ...
        <div>
          ...
          <div className="flow-empty-actions">
            <VSCodeButton variant="primary" icon={state.primaryAction.icon} onClick={() => runAction(state.primaryAction)}>
              {state.primaryAction.label}
            </VSCodeButton>
            {state.secondaryActions.map((action) => (
              <VSCodeButton key={action.command} className="compact" icon={action.icon} onClick={() => runAction(action)}>
                {action.label}
              </VSCodeButton>
            ))}
          </div>
...
function GraphEdge({ edge, nodesById, selectedId }: { edge: RenderedEdge; nodesById: Map<string, RenderedNode>; selectedId: string }) {
  ...
  const labelVisibility = edgeLabelVisibilityClass(edge, {
    active: activeEdge,
    selected: selectedEdge,
  });
  const supportEdge = isSupportEdge(edge);
  return (
    <g className={`graph-edge ${labelVisibility} ${edgeVisualPriorityClass({ active: activeEdge, selected: selectedEdge, support: supportEdge })}${edge.className ? ` ${edge.className}` : ''}${edge.animated ? ' animated' : ''}${supportEdge ? ' support-edge' : ''}${selectedId && !selectedEdge ? ' focus-muted' : ''}${selectedEdge ? ' focus-edge' : ''}`} data-edge-id={edge.id} style={{ color }}>
      <title>{title}</title>
      ...
      {edge.animated && (
        <circle className="graph-edge-tracer" r="4" fill={color}>
          <animateMotion dur="1.15s" repeatCount="indefinite" path={points.path} />
        </circle>
      )}
      {label && (
        <g className="graph-edge-label" transform={`translate(${points.labelX} ${points.labelY})`}>
...
function DebugOverlay({ status, stateVersion, draft }: { status?: FlowRenderStatus; stateVersion: number; draft: AgentPipeline }) {
  ...
  return (
    <aside className="debug-overlay" aria-label="Agent Flow debug overlay">
...
function Inspector({ node, pipeline, toolOptions, runtime, findings, conflict, onChange, onConnect, onApplyExternalChanges, onKeepLocalEdit, onOpenConflictDiff, onCancelLocalEdit }: { node?: PipelineNode; pipeline: AgentPipeline; toolOptions: ToolOptionGroup[]; runtime?: NodeRuntimeState; findings: ValidationFinding[]; conflict?: EditingConflict; onChange: (nodeId: string, patch: Partial<PipelineNode>) => void; onConnect: (sourceId: string, targetId: string, kind: ConnectionIntentKind) => void; onApplyExternalChanges: () => void; onKeepLocalEdit: () => void; onOpenConflictDiff: () => void; onCancelLocalEdit: () => void }) {
  ...
  const setOptionalString = (field: string, value: string) =>
    onChange(node.id, {
      [field]: optionalTextValue(value),
  ...
  const toggleListItem = (field: string, item: string, checked: boolean) => {
    ...
    onChange(node.id, {
      [field]: checked ? [...new Set([...current, item])] : current.filter((value) => value !== item),
    } as Partial<PipelineNode>);
  };
  const toggleArtifact = (field: 'inputs' | 'outputs' | 'requiredArtifacts', path: string, checked: boolean, action: ArtifactAction) => {
    ...
    const artifactUsages = checked
      ...
      : removeArtifactUsageIfUnselected(
          (node as any).artifactUsages,
          path,
          current.filter((value) => value !== path),
          node,
        );
    onChange(node.id, {
      [field]: checked ? [...new Set([...current, path])] : current.filter((value) => value !== path),
      artifactUsages,
    } as Partial<PipelineNode>);
  };
  const updateArtifactUsage = (path: string, patch: Partial<ArtifactUsage>, action: ArtifactAction) => {
    onChange(node.id, {
      artifactUsages: upsertArtifactUsage((node as any).artifactUsages, path, action, patch),
    } as Partial<PipelineNode>);
  };
  ...
  const updateInstructionRef = (target: string, instruction: string) => {
    onChange(node.id, {
      instructionRefs: upsertInstructionRef((node as any).instructionRefs, target, instruction),
    } as Partial<PipelineNode>);
  };
  ...
  const updateRoleRef = (target: string, instruction: string) => {
    onChange(node.id, {
      roleRefs: upsertRoleRef((node as any).roleRefs, target, instruction),
    } as Partial<PipelineNode>);
  };
  const toolGroups =
    node.type === 'agent' || node.type === 'prompt'
      ? partitionConfiguredTools({
          availableTools: flattenToolOptionValues(toolOptions),
          configuredTools: node.tools ?? [],
        })
  ...
  const syncStatus = deriveInspectorSyncStatus({
    runtime,
  ...
  if (node.type === 'handoff') {
    ...
    return (
      <div className="config">
        <InspectorHeader node={node} filePath={sourceFilePath} syncStatus={syncStatus} />
        <InspectorSection id="routing" title="Derived handoff" summary={targetAgent?.label ?? node.targetAgent ?? 'no target'} defaultOpen fieldHint="agent frontmatter handoffs">
          ...
          <dl>
            <dt>Source agent</dt>
            <dd>{sourceAgent?.label ?? node.sourceAgent ?? 'Unknown'}</dd>
            <dt>Target agent</dt>
            <dd>{targetAgent?.label ?? node.targetAgent ?? 'Unknown'}</dd>
            {node.prompt && (
              <>
                <dt>Prompt</dt>
                <dd>{node.prompt}</dd>
        ...
        <InspectorSection id="findings" title="Health" summary={findings.length ? `${findings.length} finding${findings.length === 1 ? '' : 's'}` : 'no findings'} defaultOpen={findings.length > 0}>
          {findings.length ? (
            findings.map((finding) => (
              <p key={`${finding.ruleId}-${finding.message}`} className={`inspector-finding ${finding.severity}`}>
  ...
  return (
    <div className="config">
      <InspectorHeader node={node} filePath={filePath} syncStatus={syncStatus} />
      ...
      <InspectorSection id="connections" title="Connections" summary="guided edge creation">
        <GuidedConnectionPanel node={node} pipeline={pipeline} onConnect={onConnect} />
      </InspectorSection>
      {node.type === 'agent' && (
        <InspectorSection id="run" title="Run behavior" summary={`${node.model || 'auto model'} · ${node.target || 'both environments'}`} defaultOpen>
          <label>
            Argument hint
            <input value={node.argumentHint ?? ''} onChange={(event: any) => setOptionalString('argumentHint', event.target.value)} />
          </label>
          <label>
            Model
            <input value={node.model ?? ''} onChange={(event: any) => setOptionalString('model', event.target.value)} />
          </label>
          <label>
            Target
            <select value={node.target ?? ''} onChange={(event: any) => setOptionalString('target', event.target.value)}>
          ...
          <label className="inline-check">
            <input
              type="checkbox"
              checked={node.userInvocable ?? true}
              onChange={(event: any) =>
                onChange(node.id, {
                  userInvocable: event.target.checked ? undefined : false,
                } as Partial<PipelineNode>)
              }
            />{' '}
            User invocable
          ...
          <label className="inline-check">
            <input
              type="checkbox"
              checked={node.disableModelInvocation ?? false}
              onChange={(event: any) =>
                onChange(node.id, {
                  disableModelInvocation: event.target.checked || undefined,
                } as Partial<PipelineNode>)
              }
            />{' '}
      ...
      {(node.type === 'agent' || node.type === 'prompt') && (
        <InspectorSection id="tools" title="Tools" summary={selectedToolSummaryText}>
          <ToolSelectionSummary selected={node.tools ?? []} groups={toolOptions} unavailable={toolGroups.unavailable} />
          <ToolTree groups={toolOptions} selected={node.tools ?? []} unavailable={toolGroups.unavailable} onToggle={(tool, checked) => toggleListItem('tools', tool, checked)} />
        </InspectorSection>
      )}
      {node.type === 'agent' && (
        <InspectorSection id="routing" title="Routing" summary={`${node.handoffs?.length ?? 0} handoffs · ${node.calls?.length ?? 0} subagents`}>
          <h4>Subagents</h4>
          <div className="checks">
            {agents.map((agent) => (
              <label key={agent.id}>
                <input type="checkbox" checked={(node.calls ?? []).includes(agent.id)} onChange={(event: any) => toggleListItem('calls', agent.id, event.target.checked)} />
                {agent.label}
              </label>
            ))}
          </div>
          <HandoffEditor handoffs={node.handoffs ?? []} agents={agents} onChange={setHandoffs} />
        </InspectorSection>
      )}
      {(node.type === 'agent' || node.type === 'prompt') && (
        <InspectorSection id="context" title="Context" summary={`${((node as any).instructionRefs ?? []).length} instruction · ${((node as any).roleRefs ?? []).length} role refs`}>
          <InstructionReferenceSelector instructions={instructions} refs={(node as any).instructionRefs ?? []} references={references} onToggle={toggleInstructionRef} onInstructionChange={updateInstructionRef} />
          <RoleReferenceSelector roles={roles} refs={(node as any).roleRefs ?? []} references={references} onToggle={toggleRoleRef} onRoleChange={updateRoleRef} />
        </InspectorSection>
      )}
      {node.type === 'agent' && (
        <InspectorSection id="artifacts" title="Artifacts" summary={`${(node.inputs ?? []).length} input · ${(node.outputs ?? []).length} output`}>
          <AgentArtifactSelector artifacts={artifacts} inputs={node.inputs ?? []} outputs={node.outputs ?? []} usages={node.artifactUsages ?? []} references={references} onInputToggle={(path, checked) => toggleArtifact('inputs', path, checked, 'read')} onOutputToggle={(path, checked) => toggleArtifact('outputs', path, checked, 'write')} onUsageChange={(path, patch, action) => updateArtifactUsage(path, patch, action)} />
        </InspectorSection>
      )}
      {node.type === 'prompt' && (
        <InspectorSection id="run" title="Run behavior" summary={node.startAgent || 'current agent'} defaultOpen>
          <label>
            Argument hint
            <input value={node.argumentHint ?? ''} onChange={(event: any) => setOptionalString('argumentHint', event.target.value)} />
          </label>
          ...
          <label>
            Agent
            <select
              value={node.startAgent ?? ''}
              onChange={(event: any) =>
                onChange(node.id, {
                  startAgent: event.target.value || undefined,
                } as Partial<PipelineNode>)
              }
            >
      ...
      {node.type === 'prompt' && (
        <InspectorSection id="artifacts" title="Artifacts" summary={`${node.requiredArtifacts?.length ?? 0} required`}>
          <ArtifactSelector title="Required artifacts" artifacts={artifacts} selected={node.requiredArtifacts ?? []} usages={node.artifactUsages ?? []} references={references} defaultAction="read" actionOptions={['read', 'validate']} onToggle={(path, checked) => toggleArtifact('requiredArtifacts', path, checked, 'read')} onUsageChange={(path, patch) => updateArtifactUsage(path, patch, 'read')} />
        </InspectorSection>
      )}
      {node.type === 'instruction' && (
        <InspectorSection id="context" title="Context" summary={node.applyTo || 'manual scope'} defaultOpen>
          <label>
            applyTo
            <input value={node.applyTo ?? ''} onChange={(event: any) => setOptionalString('applyTo', event.target.value)} />
          </label>
          <label>
            Exclude agent
            <select value={node.excludeAgent ?? ''} onChange={(event: any) => setOptionalString('excludeAgent', event.target.value)}>
      ...
      {(node.type === 'instruction' || node.type === 'skill') && (
        <InspectorSection id="artifacts" title="Artifacts" summary={`${node.requiredArtifacts?.length ?? 0} referenced`}>
          <ArtifactSelector title="Artifacts" artifacts={artifacts} selected={node.requiredArtifacts ?? []} usages={node.artifactUsages ?? []} references={references} defaultAction="read" actionOptions={['read', 'write', 'append', 'validate']} onToggle={(path, checked) => toggleArtifact('requiredArtifacts', path, checked, 'read')} onUsageChange={(path, patch) => updateArtifactUsage(path, patch, 'read')} />
        </InspectorSection>
      )}
      {node.type === 'skill' && (
        <InspectorSection id="run" title="Run behavior" summary={node.context || 'inline'} defaultOpen>
          <label>
            Argument hint
            <input value={node.argumentHint ?? ''} onChange={(event: any) => setOptionalString('argumentHint', event.target.value)} />
          </label>
          ...
          <label>
            Context
            <select value={node.context ?? ''} onChange={(event: any) => setOptionalString('context', event.target.value)}>
      ...
      {node.type === 'role' && (
        <InspectorSection id="context" title="Context" summary={node.roleFile ?? `.github/roles/${node.id}.md`} defaultOpen>
          <label>
            Path
            <input value={node.roleFile ?? `.github/roles/${node.id}.md`} onChange={(event: any) => setOptionalString('roleFile', event.target.value)} />
          </label>
        </InspectorSection>
      )}
      {node.type === 'artifact' && (
        <InspectorSection id="artifacts" title="Artifacts" summary={node.path} defaultOpen>
          <label>
            Path
            <input
              value={node.path}
      ...
      {node.type === 'gate' && (
        <InspectorSection id="routing" title="Routing" summary={node.condition} defaultOpen>
          <label>
            Condition
            <input
          ...
          <label>
            True branch
            <select value={node.trueBranch ?? ''} onChange={(event: any) => setOptionalString('trueBranch', event.target.value)}>
              <option value="">None</option>
          ...
          <label>
            False branch
            <select value={node.falseBranch ?? ''} onChange={(event: any) => setOptionalString('falseBranch', event.target.value)}>
              <option value="">None</option>
              {branchTargets.map((target) => (
                <option key={target.id} value={target.id}>
                  {target.label}
          ...
          <label>
            Error branch
            <select value={node.errorBranch ?? ''} onChange={(event: any) => setOptionalString('errorBranch', event.target.value)}>
              <option value="">None</option>
              {branchTargets.map((target) => (
                <option key={target.id} value={target.id}>
                  {target.label}
          ...
          <label>
            ...
            <input
              ...
              onChange={(event: any) =>
                onChange(node.id, {
                  maxIterations: event.target.value === '' ? undefined : Number(event.target.value),
                } as Partial<PipelineNode>)
              }
            />
          </label>
      ...
      {node.type === 'hook' && (
        <InspectorSection id="run" title="Run behavior" summary={node.trigger || 'manual trigger'} defaultOpen>
          <label>
            Trigger
            <input value={node.trigger ?? ''} onChange={(event: any) => setOptionalString('trigger', event.target.value)} />
          </label>
          <label>
            Action
            <textarea value={node.action ?? ''} onChange={(event: any) => setOptionalString('action', event.target.value)} />
          </label>
        </InspectorSection>
      )}
      {node.type === 'mcp-server' && (
        <InspectorSection id="run" title="Run behavior" summary={node.command || 'no command'} defaultOpen>
          <label>
            Command
            <input value={node.command ?? ''} onChange={(event: any) => setOptionalString('command', event.target.value)} />
          </label>
          <label>
            Args
            <input value={Array.isArray(node.args) ? node.args.join(' ') : (node.args ?? '')} onChange={(event: any) => setOptionalString('args', event.target.value)} />
          </label>
        </InspectorSection>
      )}
      <InspectorSection id="markdown" title="Content" summary={node.markdown ? 'custom body' : 'empty'}>
        <TiptapMarkdownEditor value={node.markdown ?? ''} references={references} onChange={(value) => onChange(node.id, { markdown: value } as Partial<PipelineNode>)} />
      </InspectorSection>
      <InspectorSection id="findings" title="Health" summary={findings.length ? `${findings.length} finding${findings.length === 1 ? '' : 's'}` : 'no findings'} defaultOpen={findings.length > 0}>
        {findings.length ? (
          findings.map((finding) => (
...
function InspectorConflictBanner({ conflict, onApplyExternalChanges, onKeepLocalEdit, onOpenConflictDiff, onCancelLocalEdit }: { conflict: EditingConflict; onApplyExternalChanges: () => void; onKeepLocalEdit: () => void; onOpenConflictDiff: () => void; onCancelLocalEdit: () => void }) {
  return (
    <div className="inspector-conflict-banner" role="alert">
      <header>
        <Codicon name="warning" />
        <strong>This file changed outside Agent Flow</strong>
      </header>
      ...
      <div className="inspector-conflict-actions">
        <VSCodeButton className="compact" icon="cloud-download" onClick={onApplyExternalChanges}>
          Apply external changes
        </VSCodeButton>
        <VSCodeButton className="compact" icon="edit" onClick={onKeepLocalEdit}>
          Keep my edit
        </VSCodeButton>
        <VSCodeButton className="compact" icon="diff" onClick={onOpenConflictDiff}>
          Open diff
        </VSCodeButton>
        <VSCodeButton className="compact" icon="discard" onClick={onCancelLocalEdit}>
          Cancel local edit
        </VSCodeButton>
      </div>
    </div>
...
function RenamePreview({ preview, summary }: { preview?: RenamePreviewModel; summary?: RenamePreviewModel }) {
  ...
  return (
    <div className={`rename-preview${isSummary ? ' rename-preview-success' : ''}`} role="status">
      ...
      <dl className="rename-preview-grid">
        <dt>Label</dt>
        <dd>
          {activePreview.currentLabel} -&gt; {activePreview.nextLabel}
        </dd>
        <dt>File</dt>
        <dd>
          {activePreview.currentFile} -&gt; {activePreview.nextFile}
        </dd>
        <dt>References to update</dt>
        <dd>{activePreview.rewrittenReferenceCount}</dd>
      </dl>
      {activePreview.updatedFiles.length > 0 && (
        <div className="rename-preview-files">
          <span>{isSummary ? 'Updated files' : 'Files to update'}</span>
          {activePreview.updatedFiles.map((file) => (
...
function InspectorHeader({ filePath, node, syncStatus }: { filePath: string; node: PipelineNode; syncStatus: InspectorSyncStatus }) {
  return (
    <div className={`config-header sticky inspector-sticky-header inspector-sync-${syncStatus.kind}`}>
      <div className="inspector-node-context">
        <div className="inspector-title-row">
          <h2>{node.label}</h2>
          <span className="pill node-type-pill" style={{ background: typeColors[node.type] }}>
            {node.type}
          </span>
        </div>
        <span className="config-subtitle" title={filePath}>
          {filePath}
        </span>
        <span className="inspector-sync-status" title={syncStatus.detail}>
          <Codicon name={syncStatus.icon} />
          {syncStatus.label}
...
function InspectorSection({ children, defaultOpen = false, fieldHint, id, summary, title }: { children: React.ReactNode; defaultOpen?: boolean; fieldHint?: string; id: string; summary?: React.ReactNode; title: string }) {
  ...
  return (
    <details className={`vscode-section inspector-section ${inspectorSectionClassNames[id] ?? `inspector-section-${id}`}`} data-section-id={id} open={defaultOpen}>
      <summary>
        <Codicon name="chevron-right" />
        <span>{title}</span>
        {summary && <small className="inspector-section-summary">{summary}</small>}
        {resolvedFieldHint && <small className="inspector-section-field">Writes: {resolvedFieldHint}</small>}
      </summary>
...
function InspectorQuickActions({ hasAgent, hasArtifact, hasInstruction, node, onAddHandoff, onAddInput, onAddInstruction, onAddOutput }: { hasAgent: boolean; hasArtifact: boolean; hasInstruction: boolean; node: PipelineNode; onAddHandoff: () => void; onAddInput: () => void; onAddInstruction: () => void; onAddOutput: () => void }) {
  ...
  return (
    <div className="inspector-quick-actions" aria-label="Common node actions">
      {canReferenceArtifacts && (
        <VSCodeButton className="compact" icon="arrow-down" disabled={!hasArtifact} onClick={onAddInput}>
          Add input artifact
        </VSCodeButton>
      )}
      {node.type === 'agent' && (
        <VSCodeButton className="compact" icon="arrow-up" disabled={!hasArtifact} onClick={onAddOutput}>
          Add output artifact
        </VSCodeButton>
      )}
      {canReferenceInstructions && (
        <VSCodeButton className="compact" icon="references" disabled={!hasInstruction} onClick={onAddInstruction}>
          Add instruction
        </VSCodeButton>
      )}
      {node.type === 'agent' && (
        <VSCodeButton className="compact" icon="arrow-swap" disabled={!hasAgent} onClick={onAddHandoff}>
          Add handoff
        </VSCodeButton>
      )}
    </div>
...
function ConnectionIntentChooser({ onCancel, onCreateAndConnect, onCreateOnly, pending, source }: { onCancel: () => void; onCreateAndConnect: (kind: ConnectionIntentKind) => void; onCreateOnly: () => void; pending: PendingNodeConnection; source?: PipelineNode }) {
  ...
  return (
    <section className="connection-intent-chooser" aria-label="Connection intent chooser">
      <div className="connection-intent-heading">
        <strong>Connect new node?</strong>
        <small>
          {source?.label ?? pending.sourceId} -&gt; {pending.targetNode.label}
        </small>
      </div>
      <label>
        Intent
        <select value={intent} onChange={(event: any) => setIntent(event.target.value)}>
          {pending.options.map((option) => (
            <option className={option.enabled ? undefined : 'invalid-connection-option'} disabled={!option.enabled} key={option.kind} value={option.kind}>
              {option.label}
            </option>
          ))}
        </select>
      ...
      <div className="connection-intent-actions">
        <VSCodeButton className="compact" onClick={onCreateOnly}>
          Create without connection
        </VSCodeButton>
        <VSCodeButton className="compact" icon="add" disabled={!selectedOption?.enabled} onClick={() => selectedOption && onCreateAndConnect(selectedOption.kind)}>
          Create and connect
        </VSCodeButton>
...
function GuidedConnectionPanel({ node, onConnect, pipeline }: { node: PipelineNode; onConnect: (sourceId: string, targetId: string, kind: ConnectionIntentKind) => void; pipeline: AgentPipeline }) {
  ...
  return (
    <div className="guided-connection-panel">
      <label>
        Target
        <select value={targetId} onChange={(event: any) => setTargetId(event.target.value)}>
          {targets.map((target) => (
            <option key={target.id} value={target.id}>
      ...
      {selectedOption && <ConnectionIntentPreview option={selectedOption} />}
      <VSCodeButton className="compact" icon="add" disabled={!selectedOption?.enabled} onClick={() => selectedOption && onConnect(node.id, targetId, selectedOption.kind)}>
        Add connection
      </VSCodeButton>
    </div>
  );
...
function ConnectionIntentPreview({ option }: { option: ReturnType<typeof buildConnectionIntentOptions>[number] }) {
  return (
    <div className={`connection-intent-preview${option.enabled ? '' : ' invalid-connection-option'}`}>
      <strong>Connection preview</strong>
      <dl>
        <dt>File</dt>
        <dd>{option.preview.targetFile ?? 'Not persisted'}</dd>
        <dt>Field</dt>
        <dd>{option.preview.field}</dd>
        <dt>Write</dt>
        <dd>{option.preview.value}</dd>
        {option.preview.placeholder && (
          <>
            <dt>placeholder token</dt>
            <dd>
              <code>{option.preview.placeholder}</code>
            </dd>
          </>
        )}
      </dl>
...
function ToolSelectionSummary({ groups, selected, unavailable }: { groups: readonly ToolOptionGroup[]; selected: readonly string[]; unavailable: readonly string[] }) {
  ...
  return (
    <div className="tool-selection-summary" aria-label="Selected tools">
      {tools.slice(0, 8).map((tool) => (
        <span key={tool}>{tool}</span>
      ))}
      {tools.length > 8 && <small>+{tools.length - 8}</small>}
    </div>
  );
}

function selectedToolSummary(selected: readonly string[], groups: readonly ToolOptionGroup[], unavailable: readonly string[]): string {
  const count = selectedToolSummaryItems({
    groups,
    selected,
    unavailable,
  }).length;
    ...
    <div className="tool-tree">
      ...
      {visibleGroups.length ? (
        visibleGroups.map((group) => {
          ...
          return (
            <details className="tool-group" key={group.id} open={Boolean(query) || undefined}>
              <summary>
                {group.icon && <Codicon name={group.icon} />}
                <span>{group.label}</span>
                <small className="tool-group-count">
                  {summary.selected}/{summary.total}
                </small>
              </summary>
              <div className="tool-group-options">
                {group.options.map((option) => (
                  <ToolOptionRow key={option.value} option={option} selectedSet={selectedSet} onToggle={onToggle} />
                ))}
              </div>
            </details>
          );
        ...
        <p className="tool-search-empty">No tools match the current search.</p>
      ) : (
        <p className="hint">No VS Code language model tools are registered.</p>
      )}
      {visibleUnavailable.length > 0 && (
        <details className="tool-group unavailable-tools" open={Boolean(query) || undefined}>
          <summary>
            <Codicon name="warning" />
            <span>Selected tools</span>
            <small className="tool-group-count">
              {visibleUnavailable.length}/{visibleUnavailable.length}
            </small>
          </summary>
          <div className="tool-group-options">
            {visibleUnavailable.map((tool) => (
              <label className="tool-option-row unavailable" key={tool} title="Selected on this node, but not registered by VS Code right now.">
                <input type="checkbox" checked={true} onChange={(event: any) => onToggle(tool, event.target.checked)} />
                <span className="tool-option-icon">
                  <Codicon name="question" />
                </span>
                <span className="tool-option-text">
                  <span className="tool-option-label">{tool}</span>
                </span>
              </label>
            ))}
          </div>
...
function ToolOptionRow({ onToggle, option, parent, selectedSet }: { option: ToolOption; parent?: ToolOption; selectedSet: Set<string>; onToggle: (tool: string, checked: boolean) => void }) {
  ...
  const checkbox = (
    <div className={`tool-option-row${disabled ? ' inherited' : ''}`}>
      ...
      <span className="tool-option-icon">
        <Codicon name={option.icon ?? 'symbol-method'} />
      </span>
      <label className="tool-option-text" htmlFor={inputId}>
        <span className="tool-option-label">{option.label}</span>
      </label>
      {option.description && (
        <span className="tool-option-help" data-tooltip={option.description} tabIndex={0} aria-label={option.description} role="img">
          <Codicon name="info" />
        </span>
      )}
    </div>
  ...
  return (
    <details className="tool-option-branch">
      <summary
        onClick={(event) => {
      ...
      <div className="tool-children">
        {option.children.map((child) => (
          <ToolOptionRow key={child.value} option={child} parent={option} selectedSet={selectedSet} onToggle={onToggle} />
        ))}
      </div>
    </details>
  );
  ...
  return (
    <section className="handoff-editor">
      ...
      {handoffs.length ? (
        <div className="handoff-list">
          {handoffs.map((handoff, index) => (
            <div className="handoff-row" key={index}>
              <label>
                Label
                <input value={handoff.label} onChange={(event: any) => updateHandoff(index, { label: event.target.value })} />
              </label>
              <label>
                Agent
                <select value={handoff.agent} onChange={(event: any) => updateHandoff(index, { agent: event.target.value })}>
              ...
              <label>
                Prompt
                <textarea value={handoff.prompt ?? ''} placeholder="Optional prompt for this handoff." onChange={(event: any) => updateHandoff(index, { prompt: event.target.value })} />
              </label>
              <label>
                Model
                <input value={handoff.model ?? ''} placeholder="Optional model" onChange={(event: any) => updateHandoff(index, { model: event.target.value })} />
              </label>
              <label>
                Send
                <select
                  value={typeof handoff.send === 'boolean' ? String(handoff.send) : ''}
                  onChange={(event: any) =>
                    updateHandoff(index, {
                      send: event.target.value === '' ? undefined : event.target.value === 'true',
                    })
                  }
                >
        ...
        <p className="hint">No handoffs configured.</p>
      )}
    </section>
  );
}
    ...
    <section className="artifact-picker">
      ...
      {rows.length ? (
        <div className="reference-list">
          {rows.map((artifact) => (
            <ArtifactUsageRow key={artifact.id} actionOptions={actionOptions} checked={selected.includes(artifact.path)} defaultAction={defaultAction} label={artifact.label} path={artifact.path} usage={usages.find((usage) => usage.path === artifact.path)} references={references} onToggle={onToggle} onUsageChange={onUsageChange} />
          ))}
        </div>
      ) : (
        <p className="hint">Create an artifact node to select it here.</p>
      )}
      {selectedWithoutNode.length > 0 && (
        <>
          <p className="hint">Selected paths without an artifact node.</p>
          <div className="reference-list selected-tools">
            {selectedWithoutNode.map((path) => (
              <ArtifactUsageRow key={path} actionOptions={actionOptions} checked={true} defaultAction={defaultAction} label={path} path={path} usage={usages.find((usage) => usage.path === path)} references={references} onToggle={onToggle} onUsageChange={onUsageChange} />
            ))}
          </div>
        </>
      )}
    ...
    <section className="artifact-picker">
      ...
      {rows.length ? (
        <div className="reference-list compact-reference-list">
          {rows.map((artifact) => (
            <AgentArtifactRow key={artifact.id} checkedInput={inputs.includes(artifact.path)} checkedOutput={outputs.includes(artifact.path)} label={artifact.label} path={artifact.path} usage={usages.find((usage) => usage.path === artifact.path)} references={references} onInputToggle={onInputToggle} onOutputToggle={onOutputToggle} onUsageChange={onUsageChange} />
          ))}
        </div>
      ) : (
        <p className="hint">Create an artifact node to select it here.</p>
...
function AgentArtifactRow({ checkedInput, checkedOutput, label, onInputToggle, onOutputToggle, onUsageChange, path, references, usage }: { checkedInput: boolean; checkedOutput: boolean; label: string; onInputToggle: (path: string, checked: boolean) => void; onOutputToggle: (path: string, checked: boolean) => void; onUsageChange: (path: string, patch: Partial<ArtifactUsage>, action: ArtifactAction) => void; path: string; references: ReferenceItem[]; usage?: ArtifactUsage }) {
  ...
  return (
    <ReferenceIntentCard
      ...
      nodeType="artifact"
      path={path}
      status={checked ? 'writes Markdown block' : 'parsed from Markdown'}
      onRemove={
        checked
          ? () => {
              onInputToggle(path, false);
              onOutputToggle(path, false);
            }
          : undefined
      }
    >
      <div className="reference-intent-controls">
        <div className="direction-chips" aria-label={`Artifact direction for ${label}`}>
          <label>
            <input type="checkbox" checked={checkedInput} onChange={(event: any) => onInputToggle(path, event.target.checked)} />
            Input
          </label>
          <label>
            <input type="checkbox" checked={checkedOutput} onChange={(event: any) => onOutputToggle(path, event.target.checked)} />
            Output
          </label>
        </div>
        {checked && (
          <label className="reference-action-field">
            Action
            <select aria-label={`Action for ${label}`} value={currentAction} onChange={(event: any) => onUsageChange(path, { action: event.target.value }, event.target.value)}>
              {['read', 'write', 'append', 'validate'].map((action) => (
      ...
      {checked && (
        <div className="reference-fields">
          <div className="reference-markdown-field">
            <span className="reference-markdown-label">Instruction</span>
            <ReferenceMarkdownEditor
              ariaLabel={`Instruction for ${label}`}
              ...
              onChange={(value) => onUsageChange(path, { instruction: referenceInstructionTextValue(value) }, currentAction)}
            />
          </div>
          <ReferenceMarkdownPreview kind="artifact-ref" action={currentAction} path={path} token="$artifact" instruction={usage?.instruction} />
        </div>
      )}
    </ReferenceIntentCard>
  );
...
function ArtifactUsageRow({ actionOptions, checked, defaultAction, label, onToggle, onUsageChange, path, references, usage }: { actionOptions: ArtifactAction[]; checked: boolean; defaultAction: ArtifactAction; label: string; onToggle: (path: string, checked: boolean) => void; onUsageChange: (path: string, patch: Partial<ArtifactUsage>) => void; path: string; references: ReferenceItem[]; usage?: ArtifactUsage }) {
  ...
  return (
    <ReferenceIntentCard checked={checked} label={label} nodeType="artifact" path={path} status={checked ? 'writes Markdown block' : 'parsed from Markdown'} onSelect={() => onToggle(path, true)} onRemove={checked ? () => onToggle(path, false) : undefined}>
      {checked && (
        <div className="reference-fields">
          <label className="reference-action-field">
            Action
            <select aria-label={`Action for ${label}`} value={currentAction} onChange={(event: any) => onUsageChange(path, { action: event.target.value })}>
          ...
          <div className="reference-markdown-field">
            ...
            <ReferenceMarkdownEditor
              ...
              onChange={(value) =>
                onUsageChange(path, {
                  instruction: referenceInstructionTextValue(value),
                })
              }
            />
          </div>
  ...
  return (
    <section className="reference-picker">
      ...
      {instructions.length ? (
        ...
        <p className="hint">Create an instruction node to reference it here.</p>
      )}
      {missing.length > 0 && (
        <>
          <p className="hint">Selected instruction references without an instruction node.</p>
          <div className="reference-list selected-tools">
            {missing.map((ref) => (
              <InstructionReferenceRow key={ref.target} checked={true} reference={ref} references={references} target={ref.target} onToggle={onToggle} onInstructionChange={onInstructionChange} />
            ))}
          </div>
        </>
      )}
...
function InstructionReferenceRow({ checked, instruction, onInstructionChange, onToggle, reference, references, target }: { checked: boolean; instruction?: Extract<PipelineNode, { type: 'instruction' }>; onInstructionChange: (target: string, instruction: string) => void; onToggle: (target: string, checked: boolean) => void; reference?: ReferenceInstruction; references: ReferenceItem[]; target: string }) {
  ...
  return (
    <ReferenceIntentCard checked={checked} label={instruction?.label ?? target} missing={missing} nodeType="instruction" path={target} status={missing ? 'needs repair' : checked ? 'writes Markdown block' : 'parsed from Markdown'} onSelect={() => onToggle(target, true)} onRemove={checked ? () => onToggle(target, false) : undefined}>
      {checked && (
        <div className="reference-fields">
          <div className="reference-markdown-field">
            <span className="reference-markdown-label">Purpose</span>
            <ReferenceMarkdownEditor
              ariaLabel={`Purpose for ${target}`}
              ...
              onChange={(value) => onInstructionChange(target, value)}
            />
          </div>
          <ReferenceMarkdownPreview kind="instruction-ref" path={target} token="$instruction" instruction={reference?.instruction} />
        </div>
      )}
    </ReferenceIntentCard>
  );
  ...
  return (
    <section className="reference-picker">
      ...
      {roles.length ? (
        ...
        <p className="hint">Create a role node to reference it here.</p>
      )}
      {missing.length > 0 && (
        <>
          <p className="hint">Selected role references without a role node.</p>
          <div className="reference-list selected-tools">
            {missing.map((ref) => (
              <RoleReferenceRow key={ref.target} checked={true} reference={ref} references={references} target={ref.target} onToggle={onToggle} onRoleChange={onRoleChange} />
            ))}
          </div>
        </>
      )}
...
function RoleReferenceRow({ checked, onRoleChange, onToggle, reference, references, role, target }: { checked: boolean; onRoleChange: (target: string, instruction: string) => void; onToggle: (target: string, checked: boolean) => void; reference?: ReferenceRole; references: ReferenceItem[]; role?: Extract<PipelineNode, { type: 'role' }>; target: string }) {
  ...
  return (
    <ReferenceIntentCard checked={checked} label={role?.label ?? target} missing={missing} nodeType="role" path={target} status={missing ? 'needs repair' : checked ? 'writes Markdown block' : 'parsed from Markdown'} onSelect={() => onToggle(target, true)} onRemove={checked ? () => onToggle(target, false) : undefined}>
      {checked && (
        <div className="reference-fields">
          <div className="reference-markdown-field">
            <span className="reference-markdown-label">Purpose</span>
            <ReferenceMarkdownEditor
              ...
              onChange={(value) => onRoleChange(target, value)}
            />
          </div>
          <ReferenceMarkdownPreview kind="role-ref" path={target} token="$role" instruction={reference?.instruction} />
        </div>
      )}
    </ReferenceIntentCard>
  );
...
function ReferenceIntentCard({ checked, children, label, missing = false, nodeType, onRemove, onSelect, path, status }: { checked: boolean; children?: React.ReactNode; label: string; missing?: boolean; nodeType: PipelineNodeType | 'artifact' | 'instruction' | 'role'; onRemove?: () => void; onSelect?: () => void; path: string; status: 'writes Markdown block' | 'parsed from Markdown' | 'needs repair' }) {
  return (
    <article className={`reference-intent-card${checked ? ' selected' : ''}${missing ? ' missing' : ''}`}>
      <header className="reference-intent-header">
        <div className="reference-card-target">
          <span className="reference-selection-state">
            <Codicon name={checked ? 'check' : 'circle-outline'} />
            {checked ? 'Selected reference' : 'Available reference'}
          </span>
          <strong>{label}</strong>
          <small title={path}>{path}</small>
        </div>
        <span className="pill node-type-pill" style={{ background: typeColors[nodeType] }}>
          {nodeType}
        </span>
...
function ReferenceMarkdownPreview({ action, instruction, kind, path, token }: { action?: string; instruction?: string; kind: 'artifact-ref' | 'instruction-ref' | 'role-ref'; path: string; token: '$artifact' | '$instruction' | '$role' }) {
  ...
  return (
    <details className="reference-markdown-preview">
...
function buildReferenceItems(pipeline: AgentPipeline): ReferenceItem[] {
  const generated = pipeline.nodes.flatMap((node) => {
    ...
    if (node.type === 'instruction')
      items.push({
        label: node.instructionFile ?? `.github/instructions/${node.id}.instructions.md`,
        value: `@instruction:${node.id}`,
        type: 'instruction',
      });
    if (node.type === 'skill')
      items.push({
        label: node.skillFile ?? `.github/skills/${node.id}/SKILL.md`,
        value: `@skill:${node.id}`,
        type: 'skill',
      });
    if (node.type === 'role')
      items.push({
        label: node.roleFile ?? `.github/roles/${node.id}.md`,
        value: `@role:${node.id}`,
        type: 'role',
      });
    if (node.type === 'prompt')
      items.push({
        label: node.promptFile ?? `.github/prompts/${node.id}.prompt.md`,
...
function upsertArtifactUsage(usages: ArtifactUsage[] | undefined, path: string, action: ArtifactAction, patch: Partial<ArtifactUsage> = {}): ArtifactUsage[] {
  ...
  return existing.some((usage) => usage.path === path) ? (patchKeys.length === 0 ? existing : existing.map((usage) => (usage.path === path ? { ...usage, ...patch, path } : usage))) : [...existing, { path, action, ...patch }];
}
...
function removeArtifactUsageIfUnselected(usages: ArtifactUsage[] | undefined, path: string, nextSelected: string[], node: PipelineNode): ArtifactUsage[] | undefined {
  const stillSelected = nextSelected.includes(path) || (node.type === 'agent' && [...(node.inputs ?? []), ...(node.outputs ?? [])].filter((value) => value === path).length > 1) || ((node.type === 'prompt' || node.type === 'instruction' || node.type === 'skill') && (node.requiredArtifacts ?? []).filter((value) => value === path).length > 1);
...
function upsertInstructionRef(refs: ReferenceInstruction[] | undefined, target: string, instruction?: string): ReferenceInstruction[] {
  const existing = refs ?? [];
  if (existing.some((ref) => ref.target === target))
    return existing.map((ref) =>
      ref.target === target
        ? {
            target,
            instruction: instruction === undefined ? ref.instruction : referenceInstructionTextValue(instruction),
          }
        : ref,
    );
  return [
    ...
    {
      target,
      instruction: instruction === undefined ? undefined : referenceInstructionTextValue(instruction),
    },
  ];
}
...
function TiptapMarkdownEditor({ ariaLabel = 'TipTap Markdown editor', onChange, references, referenceToken, value, variant = 'default' }: { ariaLabel?: string; onChange: (value: string) => void; references: ReferenceItem[]; referenceToken?: ReferenceToken; value: string; variant?: 'default' | 'compact' }) {
  ...
  useEffect(() => {
    ...
    editor.commands.setContent(markdownToTiptapHtml(markdown.body), {
      emitUpdate: false,
    });
  ...
  const replaceBodyMarkdown = (next: string) => {
    ...
    onChange(fullMarkdown);
    updateQuery(next);
    editor?.commands.setContent(markdownToTiptapHtml(next), {
      emitUpdate: false,
    });
  };
  ...
  return (
    <div className={`markdown-shell tiptap-shell${variant === 'compact' ? ' compact-markdown-shell' : ''}`}>
      <div className="editor-toolbar" role="toolbar" aria-label="Markdown formatting">
        <EditorTool title="Heading 1" active={editor?.isActive('heading', { level: 1 })} onClick={() => editor?.chain().focus().toggleHeading({ level: 1 }).run()}>
          H1
        </EditorTool>
        <EditorTool title="Heading 2" active={editor?.isActive('heading', { level: 2 })} onClick={() => editor?.chain().focus().toggleHeading({ level: 2 }).run()}>
          H2
        </EditorTool>
        <EditorTool title="Heading 3" active={editor?.isActive('heading', { level: 3 })} onClick={() => editor?.chain().focus().toggleHeading({ level: 3 }).run()}>
          H3
        </EditorTool>
        ...
        {referenceToken && (
          <>
            <span className="editor-separator" />
            <EditorTool title={referenceToken.title} icon={referenceToken.icon ?? 'references'} onClick={() => insertMarkdown(referenceToken.value)} />
          </>
        )}
      </div>
      {variant !== 'compact' && frontmatter.current && (
        <details className="frontmatter-drawer">
          <summary>Frontmatter</summary>
          <textarea value={frontmatter.current} onChange={(event: any) => updateFrontmatter(event.target.value)} spellCheck={false} />
        </details>
      )}
      ...
      {suggestions.length > 0 && (
        <div className="reference-menu">
          {suggestions.map((item) => (
            <button key={`${item.type}-${item.value}`} onClick={() => insertSuggestion(item)}>
              <span>{item.label}</span>
              <small>
...
function EditorTool({ active, children, icon, title, onClick }: { active?: boolean; children?: React.ReactNode; icon?: string; title: string; onClick: () => void }) {
  return (
    <VSCodeButton type="button" className={`editor-tool${active ? ' active' : ''}`} icon={icon} title={title} aria-label={title} onMouseDown={(event: any) => event.preventDefault()} onClick={onClick}>
      {children}
    </VSCodeButton>
  );
}
  ...
  const title = (
    {
      ...
      tools: 'Tool matrix',
      risk: 'Context risk',
    } as Record<BottomTab, string>
  )[activeTab];
  return (
    <div className="diagnostics">
      <nav>
        {tabs.map((tab) => (
          <VSCodeButton key={tab} variant="ghost" className={activeTab === tab ? 'active' : ''} onClick={() => setActiveTab(tab)} title={tabMeta[tab].label} aria-label={`${tabMeta[tab].label} tab`}>
            <span className="diagnostic-tab-label">
              <Codicon name={tabMeta[tab].icon} />
      ...
      <article>
        <div className="diagnostic-heading">
          <h3>{title}</h3>
          <span>{diagnosticSummary(state, activeTab)}</span>
        </div>
        {activeTab === 'activity' && <ActivityDiagnostics events={state.activityEvents ?? []} pipeline={state.pipeline} sources={state.activitySources ?? []} onSelectNode={onSelectNode} />}
        {activeTab === 'metrics' && <MetricsDiagnostics metrics={metrics} onSelectNode={onSelectNode} />}
        {activeTab === 'attention' && <FileAttentionDiagnostics entries={fileAttention} />}
        {activeTab === 'validation' && <ValidationDiagnostics findings={state.findings} pipeline={state.pipeline} toolOptions={state.toolOptions} onApplyQuickFix={onApplyQuickFix} onSelectNode={onSelectNode} />}
        {activeTab === 'files' && <FileDiagnostics files={state.generatedFiles} />}
        {activeTab === 'tools' && <ToolDiagnostics pipeline={state.pipeline} />}
        {activeTab === 'risk' && <RiskDiagnostics pipeline={state.pipeline} risk={state.risk} />}
      </article>
    </div>
  );
}
...
function FileAttentionDiagnostics({ entries }: { entries: ReturnType<typeof aggregateFileAttention> }) {
  ...
  return (
    <div className="file-attention-list">
      {entries.map((entry) => (
        <button
          type="button"
          key={entry.path}
          className="file-attention-row"
          onClick={() =>
            vscode?.postMessage({
              command: 'openWorkspaceFile',
              path: entry.path,
        ...
        >
          <div className="file-attention-title">
            <Codicon name={entry.writes ? 'edit' : 'eye'} />
            <code>{entry.path}</code>
            <span>{Math.round(entry.heat * 100)}%</span>
          </div>
          <div className="metrics-bar">
            <span style={{ width: `${Math.max(5, Math.round(entry.heat * 100))}%` }} />
          </div>
          <small>
            {entry.reads} reads · {entry.writes} writes · {entry.events} events · {entry.tokens} tok ({entry.inputTokens} in / {entry.outputTokens} out) · {entry.nodeIds.join(', ') || 'No mapped node'}
          </small>
        </button>
      ))}
    </div>
    ...
    <div className="metrics-panel">
      <div className="metrics-cards">
        {cards.map(([label, value]) => (
          <div className="metrics-card" key={label}>
            <span>{label}</span>
            <strong>{value}</strong>
          </div>
        ))}
      ...
      <section className="metrics-section">
        ...
        <div className="diagnostic-list">
          {metrics.nodes.slice(0, 12).map((node) => (
            <button key={node.nodeId} type="button" className={`diagnostic-card activity-card ${node.failedCount ? 'error' : 'neutral'}`} onClick={() => onSelectNode(node.nodeId)}>
              <Codicon name={node.failedCount ? 'error' : 'pulse'} />
              <div>
                <div className="diagnostic-card-title">
                  <span>{node.label}</span>
                  <code>{node.eventCount} events</code>
                  {node.tokenEstimate > 0 && <code>{node.tokenEstimate} tok</code>}
                </div>
                <p>
                  {node.completedCount} completed · {node.failedCount} failed
                </p>
                <small>
                  {node.inputTokens} input · {node.outputTokens} output · {node.lastActivity ? new Date(node.lastActivity).toLocaleString() : 'No activity timestamp'}
                </small>
              </div>
            </button>
          ))}
      ...
      <section className="metrics-section">
        <h4>Top files and artifacts</h4>
        <div className="metrics-files">
          {metrics.files.slice(0, 12).map((file) => (
            <div key={file.path} className="metrics-file-row">
              <div className="metrics-file-main">
                <code>{file.path}</code>
                <span>
                  {file.reads} reads · {file.writes} writes · {file.events} events · {file.tokens} tok ({file.inputTokens} in / {file.outputTokens} out)
                </span>
              </div>
              <div className="metrics-bar">
                <span
                  style={{
                    width: `${Math.max(5, Math.round((file.events / maxFileEvents) * 100))}%`,
                  }}
                />
              </div>
  ...
  if (!events.length)
    return (
      <div className="activity-panel">
        <EmptyDiagnostics icon="pulse" title="No activity yet" detail="Agent Flow can show events from Agent Flow language model tools, VS Code document events, filesystem writes, and GitHub Copilot debug logs. File watchers alone cannot observe reads." />
        {sourceStatus}
      </div>
    );
  const labels = new Map(pipeline.nodes.map((node) => [node.id, node.label]));
  const filtered = events.filter((event) => (!filters.sessionId || event.sessionId === filters.sessionId) && (!filters.nodeId || event.nodeId === filters.nodeId) && (!filters.phase || event.phase === filters.phase) && (!filters.toolName || event.toolName === filters.toolName) && (!filters.artifactPath || event.artifactPath === filters.artifactPath) && (!filters.severity || (event.severity ?? (event.phase === 'failed' ? 'error' : 'info')) === filters.severity));
  return (
    <div className="activity-panel">
      {sourceStatus}
      <div className="activity-actions">
        <div className="activity-view-switch">
          {(['events', 'timeline', 'transcript'] as const).map((item) => (
            <VSCodeButton key={item} variant="ghost" className={view === item ? 'active' : ''} onClick={() => setView(item)}>
              {item}
            </VSCodeButton>
          ))}
        </div>
        <ActivityFilter label="Session" value={filters.sessionId} options={unique(events.map((event) => event.sessionId))} onChange={(sessionId) => setFilters((current) => ({ ...current, sessionId }))} />
        <ActivityFilter label="Node" value={filters.nodeId} options={pipeline.nodes.filter((node) => events.some((event) => event.nodeId === node.id)).map((node) => ({ value: node.id, label: node.label }))} onChange={(nodeId) => setFilters((current) => ({ ...current, nodeId }))} />
        <ActivityFilter label="Phase" value={filters.phase} options={unique(events.map((event) => event.phase))} onChange={(phase) => setFilters((current) => ({ ...current, phase }))} />
        <ActivityFilter label="Tool" value={filters.toolName} options={unique(events.map((event) => event.toolName).filter(Boolean) as string[])} onChange={(toolName) => setFilters((current) => ({ ...current, toolName }))} />
        <ActivityFilter label="Artifact" value={filters.artifactPath} options={unique(events.map((event) => event.artifactPath).filter(Boolean) as string[])} onChange={(artifactPath) => setFilters((current) => ({ ...current, artifactPath }))} />
        <ActivityFilter label="Severity" value={filters.severity} options={['info', 'warning', 'error']} onChange={(severity) => setFilters((current) => ({ ...current, severity }))} />
        <VSCodeButton className="compact" icon="clear-all" onClick={() => vscode?.postMessage({ command: 'clearActivity' })}>
          Clear activity
        </VSCodeButton>
      </div>
      ...
      {view === 'events' && (
        <div className="diagnostic-list activity-list">
          {[...filtered].reverse().map((event) => (
            <button type="button" key={event.id} className={`diagnostic-card activity-card ${event.severity === 'error' || event.phase === 'failed' ? 'error' : event.severity === 'warning' ? 'warning' : 'neutral'}`} onClick={() => event.nodeId && onSelectNode(event.nodeId)} disabled={!event.nodeId}>
              ...
              <div>
                <div className="diagnostic-card-title">
                  ...
                  {event.nodeId && <code>{labels.get(event.nodeId) ?? event.nodeId}</code>}
                  {event.toolName && <code>{event.toolName}</code>}
                </div>
                <p>{event.summary}</p>
                <small>
                  {new Date(event.timestamp).toLocaleTimeString()} · {event.sessionId}
                  {event.artifactPath ? ` · ${event.artifactPath}` : ''}
                  {event.aiCredits !== undefined ? ` · ${event.aiCredits.toFixed(3)} AI credits` : ''}
                </small>
              </div>
            </button>
          ))}
  ...
  return (
    <div className="activity-timeline">
      {timeline.sessions.map((session) => (
        <section key={session.sessionId} className={`timeline-session${session.failed ? ' failed' : ''}`}>
          <h4>
            <span>{session.sessionId}</span>
            <small>
              {session.events.length} events · {timeRange(session.startedAt, session.updatedAt)}
            </small>
          </h4>
          <div className="timeline-node-list">
            {session.nodes.map((node) => (
              <div key={node.nodeId} className={`timeline-node-group${node.failed ? ' failed' : ''}`}>
                <button type="button" className="timeline-node-title" onClick={() => onSelectNode(node.nodeId)}>
                  <Codicon name={node.failed ? 'error' : 'circle-large-filled'} />
                  <span>{labels.get(node.nodeId) ?? node.nodeId}</span>
                  <small>{node.events.length}</small>
                </button>
                <ol>
                  {node.events.map((event) => (
                    <li key={event.id}>
                      <button type="button" onClick={() => event.nodeId && onSelectNode(event.nodeId)} disabled={!event.nodeId}>
                        <span className={`timeline-dot phase-${event.phase}`} />
  ...
  return (
    <div className="activity-transcript">
      {events.map((event) => {
        ...
        return (
          <button type="button" key={event.id} className={`transcript-row phase-${event.phase}`} onClick={() => event.nodeId && onSelectNode(event.nodeId)} disabled={!event.nodeId}>
            <span>{speaker}</span>
            <p>{event.summary}</p>
            <small>
...
function ActivitySourceStatuses({ sources }: { sources: ActivitySourceRuntimeState[] }) {
  ...
  return (
    <div className="activity-source-grid">
      {sources.map((source) => (
        <ActivitySourceStatus source={source} key={source.id} />
      ))}
    </div>
  );
}
...
function ActivitySourceStatus({ source }: { source: ActivitySourceRuntimeState }) {
  ...
  return (
    <div className={`activity-source-status ${source.state}`}>
      <div className="diagnostic-card-title">
        <Codicon name={icon} />
      ...
      <p>{source.detail}</p>
      {source.id === 'copilotDebugLogs' && source.metadata?.copilotFileLoggingEnabled === false && (
        <p className="diagnostic-muted">
          Enable <code>github.copilot.chat.agentDebugLog.fileLogging.enabled</code> in VS Code settings to import sanitized Copilot activity.
        </p>
      )}
      {roots.length > 0 && (
        <div className="diagnostic-chip-row">
          {roots.slice(0, 3).map((root) => (
            <span className="diagnostic-chip" key={root}>
              {root}
            </span>
          ))}
        </div>
...
function ValidationDiagnostics({ findings, onApplyQuickFix, onSelectNode, pipeline, toolOptions }: { findings: ValidationFinding[]; onApplyQuickFix: (action: ValidationAction | undefined) => void; onSelectNode: (nodeId: string) => void; pipeline: AgentPipeline; toolOptions: ToolOptionGroup[] }) {
  ...
  if (!findings.length)
    return (
      <>
        <ReadyToRunSummary findings={[]} />
        <EmptyDiagnostics icon="pass" title="No findings" detail="The inferred flow has no validation warnings right now." />
      </>
    );
  return (
    <div className="validation-workflow">
      ...
      <div className="diagnostic-list">
        {filtered.map((finding, index) => (
          <ActionableDiagnosticCard key={`${finding.finding.ruleId}-${index}`} finding={finding} onApplyQuickFix={() => onApplyQuickFix(finding.quickFix)} onFocusNode={() => finding.finding.nodeId && openInspectorSection(finding.finding.nodeId, finding.sectionId, onSelectNode)} />
        ))}
      </div>
    </div>
  );
...
function ReadyToRunSummary({ findings }: { findings: ValidationFinding[] }) {
  ...
  return (
    <section className={`validation-ready-summary ${summary.state}`}>
      <Codicon name={summary.state === 'pass' ? 'pass' : summary.state === 'warn' ? 'warning' : 'error'} />
      <div>
        <strong>{summary.title}</strong>
        <span>
          {summary.errors} errors · {summary.warnings} warnings · {summary.risks} risks · {summary.infos} info
          {summary.infos === 1 ? '' : 's'}
        </span>
      </div>
    </section>
...
function DiagnosticFilters({ entityTypes, onSeverityChange, onTypeChange, severityFilter, typeFilter }: { entityTypes: string[]; onSeverityChange: (value: string) => void; onTypeChange: (value: string) => void; severityFilter: string; typeFilter: string }) {
  return (
    <div className="validation-filter-bar">
      <ActivityFilter label="Severity" value={severityFilter} options={['error', 'warning', 'risk', 'info']} onChange={onSeverityChange} />
      <ActivityFilter label="Entity" value={typeFilter} options={entityTypes} onChange={onTypeChange} />
    </div>
  );
}
...
function ActionableDiagnosticCard({ finding, onApplyQuickFix, onFocusNode }: { finding: ActionableDiagnostic; onApplyQuickFix: () => void; onFocusNode: () => void }) {
  ...
  return (
    <div className={`diagnostic-card diagnostic-workflow-card ${validation.severity}`}>
      <Codicon name={validation.severity === 'error' ? 'error' : validation.severity === 'warning' ? 'warning' : validation.severity === 'risk' ? 'flame' : 'info'} />
      <div>
        <div className="diagnostic-card-title">
          <span>{finding.title}</span>
          <code>{finding.entity}</code>
          <code>{validation.ruleId}</code>
        </div>
        <p>{validation.message}</p>
        <dl className="diagnostic-explainer">
        ...
        {finding.savedTools && (
          <div className="diagnostic-tool-details">
            <span>
              Saved tool ids <code>{finding.savedTools.join(', ') || 'none'}</code>
            </span>
            <span>
              Normalized tool ids <code>{finding.normalizedTools?.join(', ') || 'none'}</code>
            </span>
            <span>
              VS Code registered <code>{finding.registeredTools?.join(', ') || 'none'}</code>
            </span>
          </div>
        )}
        <div className="diagnostic-actions">
          <VSCodeButton className="compact" icon="target" disabled={!validation.nodeId} onClick={onFocusNode}>
            Focus node
          </VSCodeButton>
          <VSCodeButton
            className="compact"
            icon="go-to-file"
            disabled={!finding.entityPath}
            onClick={() =>
              finding.entityPath &&
              vscode?.postMessage({
                command: 'openWorkspaceFile',
                path: finding.entityPath,
              })
            }
          >
            Open file
          </VSCodeButton>
          <VSCodeButton className="compact" icon="sparkle" disabled={!finding.quickFix} onClick={onApplyQuickFix} title={finding.quickFix ? 'Apply a safe deterministic edit for this finding.' : 'Quick fixes are shown when Agent Flow can make a safe deterministic edit.'}>
            Apply quick fix
          </VSCodeButton>
        </div>
      </div>
...
function buildActionableDiagnostic(finding: ValidationFinding, pipeline: AgentPipeline, toolOptions: ToolOptionGroup[]): ActionableDiagnostic {
  ...
  const base = {
    ...
    registeredTools,
    savedTools,
  } satisfies Partial<ActionableDiagnostic>;
  if (finding.ruleId === 'broad-apply-to' || finding.ruleId === 'markdown-apply-to')
    return {
      ...base,
      title: 'Broad instruction scope',
      why: 'This instruction can silently apply to many Copilot customization files and consume context where it is not intended.',
      fix: `Narrow applyTo on ${entityPath ?? 'the instruction'} to the target folder or file pattern.`,
      sectionId: 'context',
    } as ActionableDiagnostic;
  if (finding.ruleId === 'agent-no-output')
    return {
      ...base,
      title: 'Missing output artifact',
      why: 'The next agent or prompt has no explicit handoff file to read, so work can disappear into chat context.',
      fix: 'Create or select an output artifact and describe what this node should write to it.',
      sectionId: 'artifacts',
    } as ActionableDiagnostic;
  if (finding.ruleId.includes('tool') || finding.ruleId.includes('command') || finding.ruleId.includes('edit'))
    return {
      ...base,
      title: 'Tool access needs review',
      why: 'Tool permissions define what Copilot can read, write, execute, or delegate from this node.',
      fix: 'Review the selected tools and add a command/edit safety policy when broad tools are required.',
      sectionId: 'tools',
    } as ActionableDiagnostic;
  if (finding.ruleId.includes('artifact'))
    return {
      ...base,
      title: 'Artifact boundary issue',
      why: 'Artifact read/write edges are how the flow preserves context between nodes.',
      fix: 'Add the missing producer or consumer, or remove the unused artifact reference.',
      sectionId: 'artifacts',
    } as ActionableDiagnostic;
  if (finding.ruleId.includes('subagent') || finding.ruleId.includes('agent'))
    return {
      ...base,
      title: 'Routing reference issue',
      why: 'Broken agent references stop the flow from reaching the intended target node.',
      fix: 'Select an existing target agent or rename the reference to match the target file.',
      sectionId: 'routing',
    } as ActionableDiagnostic;
  return {
    ...base,
    title: 'Validation finding',
    why: 'This finding can affect pipeline correctness, maintainability, or runtime behavior.',
    fix: 'Review the referenced node or file and update the configuration before publishing.',
    sectionId: node ? 'identity' : undefined,
  } as ActionableDiagnostic;
}
...
function FileDiagnostics({ files }: { files: State['generatedFiles'] }) {
  ...
  return (
    <div className="diagnostic-file-groups">
      {groups.map(([kind, paths]) => (
        <section key={kind} className="diagnostic-file-group">
          <h4>
            <span>{kind}</span>
...
function ToolDiagnostics({ pipeline }: { pipeline: AgentPipeline }) {
  ...
  return (
    <div className="diagnostic-list">
      {nodes.map((node) => (
        <div key={node.id} className="diagnostic-card neutral">
          <Codicon name={node.type === 'agent' ? 'hubot' : 'comment-discussion'} />
          <div>
            <div className="diagnostic-card-title">
              <span>{node.label}</span>
              <code>{node.type}</code>
            ...
            {node.tools?.length ? (
              <div className="diagnostic-chip-row">
                {node.tools.map((tool) => (
                  <span key={tool} className="diagnostic-chip">
                    {tool}
                  </span>
                ))}
              </div>
            ) : (
              <p className="diagnostic-muted">No tools selected.</p>
            )}
          </div>
        </div>
      ))}
        ...
        <ul>
          {risk.reasons.map((reason) => (
            <li key={reason}>
              ...
              <div>
                <span>{reason}</span>
                <RiskReasonDetails pipeline={pipeline} reason={reason} />
              </div>
            </li>
          ))}
        </ul>
      ) : (
        <p className="diagnostic-muted">No risk reasons reported.</p>
      )}
    </div>
  );
}
...
function RiskReasonDetails({ pipeline, reason }: { pipeline: AgentPipeline; reason: string }) {
  ...
  return (
    <div className="diagnostic-risk-details">
      {details.map((detail) => (
...
function riskReasonDetails(pipeline: AgentPipeline, reason: string): string[] {
  if (reason.includes('broad applyTo')) {
    return pipeline.nodes.filter((node): node is Extract<PipelineNode, { type: 'instruction' }> => node.type === 'instruction' && Boolean(node.applyTo && ['**/*', '**/*.md'].includes(node.applyTo))).map((node) => `${node.label} (${node.instructionFile ?? `${node.id}.instructions.md`}, applyTo ${node.applyTo})`);
  }
  if (reason.includes('can run commands')) {
    return pipeline.nodes.filter((node): node is Extract<PipelineNode, { type: 'agent' }> => node.type === 'agent' && Boolean(node.tools?.includes('execute') || node.tools?.includes('runCommands'))).map((node) => `${node.label} (${node.agentFile ?? `${node.id}.agent.md`})`);
  }
  if (reason.includes('generic descriptions')) {
    return pipeline.nodes.filter((node): node is Extract<PipelineNode, { type: 'skill' }> => node.type === 'skill' && (!node.description || /general|helpful|useful/i.test(node.description))).map((node) => `${node.label} (${node.skillFile ?? `${node.id}/SKILL.md`})`);
  }
  if (reason.includes('embedded examples or samples')) {
    return pipeline.nodes.filter((node): node is Extract<PipelineNode, { type: 'skill' }> => node.type === 'skill' && Boolean(node.procedure?.some((step) => /example|sample/i.test(step)))).map((node) => `${node.label} (${node.skillFile ?? `${node.id}/SKILL.md`})`);
  }
  if (reason.includes('context budget')) {
    return pipeline.nodes.filter((node): node is Extract<PipelineNode, { type: 'agent' }> => node.type === 'agent' && !node.contextBudget?.length).map((node) => `${node.label} (${node.agentFile ?? `${node.id}.agent.md`})`);
  }
  if (reason.includes('input or output artifact boundaries')) {
    return pipeline.nodes.filter((node): node is Extract<PipelineNode, { type: 'agent' }> => node.type === 'agent' && (!node.inputs?.length || !node.outputs?.length)).map((node) => `${node.label} (${node.agentFile ?? `${node.id}.agent.md`})`);
  }
