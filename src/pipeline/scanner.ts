...
function frontmatter(source: string): Record<string, FrontmatterValue> {
  ...
  for (const marker of [GENERATED_MARKER, '<!-- Generated by AgentFlow. Manual changes may be overwritten. -->']) {
...
function stripGeneratedMarker(source: string): string {
  ...
  for (const marker of [GENERATED_MARKER, '<!-- Generated by AgentFlow. Manual changes may be overwritten. -->']) {
    if (content.trimEnd().endsWith(marker)) content = content.trimEnd().slice(0, -marker.length).replace(/(?:\r?\n)+$/, '');
  }
  return content;
}
...
async function hydrateMarkdownContent(workspace: string, pipeline: AgentPipeline): Promise<AgentPipeline> {
  ...
  await mergeWorkspaceCustomizationFiles(workspace, nodes, edges);
...
function explicitCustomizationFileForNode(node: PipelineNode): string | undefined {
...
async function mergeWorkspaceCustomizationFiles(workspace: string, nodes: PipelineNode[], edges: PipelineEdge[]): Promise<void> {
  const inferred = await inferPipelineFromWorkspace(workspace);
  const inferredFileNodes: PipelineNode[] = [];
  for (const node of inferred.nodes) {
    if (!isCustomizationFileNode(node)) continue;
    const file = explicitCustomizationFileForNode(node);
    if (!file || !(await exists(path.join(workspace, file)))) continue;
    inferredFileNodes.push(node);
  }

  const existingById = new Map(nodes.map((node) => [node.id, node]));
  const existingByFile = new Map<string, PipelineNode>();
  for (const node of nodes) {
    const file = explicitCustomizationFileForNode(node);
    if (file) existingByFile.set(file, node);
  }

  const idMap = new Map<string, string>();
  const renameCandidates = await staleRenameCandidates(workspace, nodes, inferredFileNodes);

  for (const inferredNode of inferredFileNodes) {
    const inferredFile = explicitCustomizationFileForNode(inferredNode);
    if (!inferredFile) continue;

    const existingForFile = existingByFile.get(inferredFile);
    if (existingForFile) {
      idMap.set(inferredNode.id, existingForFile.id);
      continue;
    }

    const existingForId = existingById.get(inferredNode.id);
    if (existingForId?.type === inferredNode.type) {
      replaceNode(nodes, existingForId.id, mergeFileBackedNode(existingForId, inferredNode));
      existingByFile.set(inferredFile, existingForId);
      idMap.set(inferredNode.id, existingForId.id);
      continue;
    }

    const renameCandidate = renameCandidates.get(inferredNode.id);
    if (renameCandidate) {
      const merged = mergeFileBackedNode(renameCandidate, inferredNode);
      replaceNode(nodes, renameCandidate.id, merged);
      existingByFile.set(inferredFile, merged);
      idMap.set(inferredNode.id, renameCandidate.id);
      continue;
    }

    const added = { ...inferredNode, position: nextHydratedPosition(nodes) };
    nodes.push(added);
    existingById.set(added.id, added);
    existingByFile.set(inferredFile, added);
    idMap.set(inferredNode.id, added.id);
  }

  for (const edge of inferred.edges) {
    ...
    pushEdgeByRelation(edges, {
      ...edge,
      id: from === edge.from && to === edge.to ? edge.id : `${from}-${edge.kind}-${to}${edge.label ? `-${slugPart(edge.label)}` : ''}`,
      from,
      to
    });
  }
...
async function staleRenameCandidates(workspace: string, existingNodes: PipelineNode[], inferredNodes: PipelineNode[]): Promise<Map<string, PipelineNode>> {
  ...
  for (const type of ['agent', 'prompt', 'instruction', 'skill', 'role'] as const) {
    ...
    for (const node of existingNodes) {
      const file = explicitCustomizationFileForNode(node);
      if (node.type === type && file && !(await exists(path.join(workspace, file)))) staleExisting.push(node);
    }
    const newInferred = inferredNodes.filter((node) => node.type === type && !existingFiles.has(explicitCustomizationFileForNode(node) ?? ''));
    if (staleExisting.length === 1 && newInferred.length === 1) matches.set(newInferred[0].id, staleExisting[0]);
  }
  return matches;
}

function mergeFileBackedNode(existing: PipelineNode, inferred: PipelineNode): PipelineNode {
  return { ...inferred, id: existing.id, position: existing.position ?? inferred.position };
}

function replaceNode(nodes: PipelineNode[], id: string, nextNode: PipelineNode): void {
...
function pushEdgeByRelation(edges: PipelineEdge[], edge: PipelineEdge): void {
  if (!edges.some((item) => item.from === edge.from && item.to === edge.to && item.kind === edge.kind && item.label === edge.label)) edges.push(edge);
}
